...
public class Cubic {
    ...
    private Cubic(long anchor0, long control0, long control1, long anchor1) {
        this(new float[]{PointKt.m8475getXDnnuFBc(anchor0), PointKt.m8476getYDnnuFBc(anchor0), PointKt.m8475getXDnnuFBc(control0), PointKt.m8476getYDnnuFBc(control0), PointKt.m8475getXDnnuFBc(control1), PointKt.m8476getYDnnuFBc(control1), PointKt.m8475getXDnnuFBc(anchor1), PointKt.m8476getYDnnuFBc(anchor1)});
    }

    /* JADX INFO: renamed from: pointOnCurve-OOQOV4g$graphics_shapes_release, reason: not valid java name */
    public final long m8463pointOnCurveOOQOV4g$graphics_shapes_release(float t) {
        float u = 1.0f - t;
        return FloatFloatPair.m10constructorimpl((getAnchor0X() * u * u * u) + (getControl0X() * 3.0f * t * u * u) + (getControl1X() * 3.0f * t * t * u) + (getAnchor1X() * t * t * t), (getAnchor0Y() * u * u * u) + (getControl0Y() * 3.0f * t * u * u) + (getControl1Y() * 3.0f * t * t * u) + (getAnchor1Y() * t * t * t));
    }
    ...
    public final void calculateBounds$graphics_shapes_release(float[] bounds, boolean approximate) {
        ...
        float minX2 = Math.min(getAnchor0X(), getAnchor1X());
        float minY = Math.min(getAnchor0Y(), getAnchor1Y());
        float maxX = Math.max(getAnchor0X(), getAnchor1X());
        float maxY = Math.max(getAnchor0Y(), getAnchor1Y());
        if (approximate) {
            bounds[0] = Math.min(minX2, Math.min(getControl0X(), getControl1X()));
            bounds[1] = Math.min(minY, Math.min(getControl0Y(), getControl1Y()));
            bounds[2] = Math.max(maxX, Math.max(getControl0X(), getControl1X()));
            bounds[3] = Math.max(maxY, Math.max(getControl0Y(), getControl1Y()));
            return;
        }
        float xa = (((-getAnchor0X()) + (getControl0X() * 3.0f)) - (getControl1X() * 3.0f)) + getAnchor1X();
        float xb = ((getAnchor0X() * 2.0f) - (getControl0X() * 4.0f)) + (getControl1X() * 2.0f);
        ...
        if (zeroIsh(xa)) {
            if (xb == 0.0f) {
                ...
                if (!(0.0f <= t && t <= 1.0f)) {
                    ...
                    float it = PointKt.m8475getXDnnuFBc(m8463pointOnCurveOOQOV4g$graphics_shapes_release(t));
                    if (it < minX2) {
                        minX2 = it;
                    }
                    if (it > maxX) {
            ...
            if (xs < 0.0f) {
                ...
                c3 = 1;
                c4 = 0;
                float t1 = ((-xb) + ((float) Math.sqrt(xs))) / (2.0f * xa);
                if (0.0f <= t1 && t1 <= 1.0f) {
                    float it2 = PointKt.m8475getXDnnuFBc(m8463pointOnCurveOOQOV4g$graphics_shapes_release(t1));
                    if (it2 < minX2) {
                        minX2 = it2;
                    }
                    if (it2 > maxX) {
                ...
                f = 3.0f;
                f2 = 2.0f;
                float t2 = ((-xb) - ((float) Math.sqrt(xs))) / (2.0f * xa);
                if (0.0f <= t2 && t2 <= 1.0f) {
                    float it3 = PointKt.m8475getXDnnuFBc(m8463pointOnCurveOOQOV4g$graphics_shapes_release(t2));
                    if (it3 < minX2) {
                        minX2 = it3;
                    }
                    if (it3 > maxX) {
        ...
        float ya = (((-getAnchor0Y()) + (f * getControl0Y())) - (f * getControl1Y())) + getAnchor1Y();
        float yb = ((f2 * getAnchor0Y()) - (getControl0Y() * 4.0f)) + (getControl1Y() * f2);
        float yc = (-getAnchor0Y()) + getControl0Y();
        if (zeroIsh(ya)) {
            if ((yb == 0.0f ? c3 : c4) != 0) {
                minX = minX2;
            } else {
                float t3 = (f2 * yc) / ((-2.0f) * yb);
                if (((0.0f > t3 || t3 > 1.0f) ? c4 : c3) == 0) {
                    minX = minX2;
                } else {
                    float it4 = PointKt.m8476getYDnnuFBc(m8463pointOnCurveOOQOV4g$graphics_shapes_release(t3));
                    if (it4 < minY) {
                        minY = it4;
                    }
                    if (it4 > maxY) {
            ...
            if (ys < 0.0f) {
                ...
                minX = minX2;
                float t12 = ((-yb) + ((float) Math.sqrt(ys))) / (f2 * ya);
                if (((0.0f > t12 || t12 > 1.0f) ? c4 : c3) != 0) {
                    float it5 = PointKt.m8476getYDnnuFBc(m8463pointOnCurveOOQOV4g$graphics_shapes_release(t12));
                    if (it5 < minY) {
                        minY = it5;
                    }
                    if (it5 > maxY) {
                ...
                float t22 = ((-yb) - ((float) Math.sqrt(ys))) / (f2 * ya);
                if (((0.0f > t22 || t22 > 1.0f) ? c4 : c3) != 0) {
                    float it6 = PointKt.m8476getYDnnuFBc(m8463pointOnCurveOOQOV4g$graphics_shapes_release(t22));
                    if (it6 < minY) {
                        minY = it6;
                    }
                    if (it6 > maxY) {
    ...
    public final Pair<Cubic, Cubic> split(float t) {
        float u = 1.0f - t;
        long pointOnCurve = m8463pointOnCurveOOQOV4g$graphics_shapes_release(t);
        return TuplesKt.to(CubicKt.Cubic(getAnchor0X(), getAnchor0Y(), (getAnchor0X() * u) + (getControl0X() * t), (getAnchor0Y() * u) + (getControl0Y() * t), (getAnchor0X() * u * u) + (getControl0X() * 2.0f * u * t) + (getControl1X() * t * t), (getAnchor0Y() * u * u) + (getControl0Y() * 2.0f * u * t) + (getControl1Y() * t * t), PointKt.m8475getXDnnuFBc(pointOnCurve), PointKt.m8476getYDnnuFBc(pointOnCurve)), CubicKt.Cubic(PointKt.m8475getXDnnuFBc(pointOnCurve), PointKt.m8476getYDnnuFBc(pointOnCurve), (getControl0X() * u * u) + (getControl1X() * 2.0f * u * t) + (getAnchor1X() * t * t), (getControl0Y() * u * u) + (getControl1Y() * 2.0f * u * t) + (getAnchor1Y() * t * t), (getControl1X() * u) + (getAnchor1X() * t), (getControl1Y() * u) + (getAnchor1Y() * t), getAnchor1X(), getAnchor1Y()));
    }
    ...
    public final Cubic transformed(PointTransformer f) {
        ...
        ArraysKt.copyInto$default(this.points, newCubic.getPoints(), 0, 0, 0, 14, (Object) null);
    ...
    public static final class Companion {
        ...
        public final Cubic circularArc(float centerX, float centerY, float x0, float y0, float x1, float y1) {
            ...
            long rotatedP0 = Utils.m8496rotate90DnnuFBc(p0d);
            long rotatedP1 = Utils.m8496rotate90DnnuFBc(p1d);
            boolean clockwise = PointKt.m8470dotProduct5P9i7ZU(rotatedP0, x1 - centerX, y1 - centerY) >= 0.0f;
            float cosa = PointKt.m8471dotProductybeJwSQ(p0d, p1d);
            if (cosa > 0.999f) {
                return straightLine(x0, y0, x1, y1);
            }
            float k = ((((Utils.distance(x0 - centerX, y0 - centerY) * 4.0f) / 3.0f) * (((float) Math.sqrt(2.0f * (1.0f - cosa))) - ((float) Math.sqrt(1.0f - (cosa * cosa))))) / (1.0f - cosa)) * (clockwise ? 1.0f : -1.0f);
            return CubicKt.Cubic(x0, y0, (PointKt.m8475getXDnnuFBc(rotatedP0) * k) + x0, (PointKt.m8476getYDnnuFBc(rotatedP0) * k) + y0, x1 - (PointKt.m8475getXDnnuFBc(rotatedP1) * k), y1 - (PointKt.m8476getYDnnuFBc(rotatedP1) * k), x1, y1);
        }
    }
}
