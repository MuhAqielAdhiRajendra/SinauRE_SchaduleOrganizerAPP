...
public final class ConnectorKt {
    ...
    static {
        ...
        int renderIntent$iv = RenderIntent.INSTANCE.m5771getPerceptualuksYyKA();
        int i = (dst$iv << 6) | src$iv | (renderIntent$iv << 12);
        Connector connectorIdentity$ui_graphics = Connector.INSTANCE.identity$ui_graphics(ColorSpaces.INSTANCE.getSrgb());
        int src$iv2 = ColorSpaces.INSTANCE.getSrgb().getId();
        int dst$iv2 = ColorSpaces.INSTANCE.getOklab().getId();
        int renderIntent$iv2 = RenderIntent.INSTANCE.m5771getPerceptualuksYyKA();
        int i2 = (dst$iv2 << 6) | src$iv2 | (renderIntent$iv2 << 12);
        DefaultConstructorMarker defaultConstructorMarker = null;
        Connector connector = new Connector(ColorSpaces.INSTANCE.getSrgb(), ColorSpaces.INSTANCE.getOklab(), RenderIntent.INSTANCE.m5771getPerceptualuksYyKA(), defaultConstructorMarker);
        int src$iv3 = ColorSpaces.INSTANCE.getOklab().getId();
        int dst$iv3 = ColorSpaces.INSTANCE.getSrgb().getId();
        int renderIntent$iv3 = RenderIntent.INSTANCE.m5771getPerceptualuksYyKA();
        Connectors = IntObjectMapKt.mutableIntObjectMapOf(i, connectorIdentity$ui_graphics, i2, connector, (renderIntent$iv3 << 12) | (dst$iv3 << 6) | src$iv3, new Connector(ColorSpaces.INSTANCE.getOklab(), ColorSpaces.INSTANCE.getSrgb(), RenderIntent.INSTANCE.m5771getPerceptualuksYyKA(), defaultConstructorMarker));
    }

    /* JADX INFO: renamed from: connectorKey-YBCOT_4, reason: not valid java name */
    public static final int m5762connectorKeyYBCOT_4(int src, int dst, int renderIntent) {
