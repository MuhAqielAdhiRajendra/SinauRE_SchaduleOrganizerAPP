...
public class CircularPropagation extends VisibilityPropagation {
    ...
    public long getStartDelay(ViewGroup sceneRoot, Transition transition, TransitionValues startValues, TransitionValues endValues) {
        ...
        if (epicenter != null) {
            ...
            int epicenterX2 = Math.round(loc[0] + (viewGroup.getWidth() / 2) + viewGroup.getTranslationX());
            epicenterY = Math.round(loc[1] + (viewGroup.getHeight() / 2) + viewGroup.getTranslationY());
            epicenterX = epicenterX2;
        }
        float distance = distance(viewCenterX, viewCenterY, epicenterX, epicenterY);
        float maxDistance = distance(0.0f, 0.0f, viewGroup.getWidth(), viewGroup.getHeight());
