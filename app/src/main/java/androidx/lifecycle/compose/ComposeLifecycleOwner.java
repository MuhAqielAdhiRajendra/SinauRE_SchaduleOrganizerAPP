...
final class ComposeLifecycleOwner implements LifecycleOwner {
    ...
    public final void setMaxLifecycleState(Lifecycle.State value) {
        this.maxLifecycleState = value;
    ...
    private final void updateLifecycleState() {
        ...
        if (this.parentLifecycleState.ordinal() < this.maxLifecycleState.ordinal()) {
            targetState = this.parentLifecycleState;
        } else {
            targetState = this.maxLifecycleState;
        }
        if (this.lifecycleRegistry.getState() == Lifecycle.State.INITIALIZED && targetState == Lifecycle.State.DESTROYED) {
            return;
        }
        this.lifecycleRegistry.setCurrentState(targetState);
    }
}
