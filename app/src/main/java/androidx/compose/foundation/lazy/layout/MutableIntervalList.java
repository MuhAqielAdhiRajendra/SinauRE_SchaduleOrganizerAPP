...
public final class MutableIntervalList<T> implements IntervalList<T> {
    ...
    public void forEach(int fromIndex, int toIndex, Function1<? super IntervalList.Interval<? extends T>, Unit> block) {
        ...
        int itemIndex = this.intervals.content[intervalIndex].getStartIndex();
        while (itemIndex <= toIndex) {
            int index$iv = intervalIndex;
            IntervalList.Interval<T> interval = this.intervals.content[index$iv];
    ...
    private final IntervalList.Interval<T> getIntervalForIndex(int itemIndex) {
        ...
        Object obj = mutableVector.content[IntervalListKt.binarySearch(this.intervals, itemIndex)];
