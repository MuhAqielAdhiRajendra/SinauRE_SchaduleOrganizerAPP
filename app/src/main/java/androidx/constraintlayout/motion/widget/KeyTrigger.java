    ...
    */
    ...
    public void conditionallyFire(float pos, View child) {
        ...
        if (this.mTriggerCollisionId != UNSET) {
            ...
            boolean z = this.mFireCrossReset;
            if (in) {
                if (z) {
                    fireCross = true;
            ...
            boolean z2 = this.mFireCrossReset;
            float f = this.mFireThreshold;
            if (z2) {
                float offset = pos - f;
                float lastOffset = this.mFireLastPos - this.mFireThreshold;
                if (offset * lastOffset < 0.0f) {
                    fireCross = true;
                    this.mFireCrossReset = false;
                }
            } else if (Math.abs(pos - f) > this.mTriggerSlack) {
                this.mFireCrossReset = true;
            }
            boolean z3 = this.mFireNegativeReset;
            float f2 = this.mFireThreshold;
            if (z3) {
                float offset2 = pos - f2;
                float lastOffset2 = this.mFireLastPos - this.mFireThreshold;
                if (offset2 * lastOffset2 < 0.0f && offset2 < 0.0f) {
                    fireNegative = true;
                    this.mFireNegativeReset = false;
                }
            } else if (Math.abs(pos - f2) > this.mTriggerSlack) {
                this.mFireNegativeReset = true;
            }
            boolean z4 = this.mFirePositiveReset;
            float f3 = this.mFireThreshold;
            if (z4) {
                float offset3 = pos - f3;
                float lastOffset3 = this.mFireLastPos - this.mFireThreshold;
                if (offset3 * lastOffset3 < 0.0f && offset3 > 0.0f) {
                    firePositive = true;
                    this.mFirePositiveReset = false;
                }
            } else if (Math.abs(pos - f3) > this.mTriggerSlack) {
                this.mFirePositiveReset = true;
            }
        }
