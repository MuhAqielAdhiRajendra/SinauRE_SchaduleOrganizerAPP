...
public class Person {
    ...
    static class Api28Impl {
        ...
        static Person fromAndroidPerson(android.app.Person person) {
            IconCompat iconCompatCreateFromIcon;
            Builder name = new Builder().setName(person.getName());
            if (person.getIcon() != null) {
                iconCompatCreateFromIcon = IconCompat.createFromIcon(person.getIcon());
            } else {
                iconCompatCreateFromIcon = null;
            }
            return name.setIcon(iconCompatCreateFromIcon).setUri(person.getUri()).setKey(person.getKey()).setBot(person.isBot()).setImportant(person.isImportant()).build();
        }
