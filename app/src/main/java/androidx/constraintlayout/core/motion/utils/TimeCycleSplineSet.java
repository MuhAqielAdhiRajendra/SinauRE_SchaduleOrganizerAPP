...
public abstract class TimeCycleSplineSet {
    ...
    protected float calcWave(float period) {
        switch (this.mWaveShape) {
            case 1:
                return Math.signum(sVal2PI * period);
            case 2:
            ...
            case 5:
                return (float) Math.cos(sVal2PI * period);
            case 6:
                float x = 1.0f - Math.abs(((period * 4.0f) % 4.0f) - 2.0f);
                return 1.0f - (x * x);
            default:
                return (float) Math.sin(sVal2PI * period);
        }
    }
    ...
    public static class CustomSet extends TimeCycleSplineSet {
        ...
        public boolean setProperty(MotionWidget view, float t, long time, KeyCache cache) {
            ...
            long delta_time = time - this.mLastTime;
            if (Float.isNaN(this.mLastCycle)) {
                this.mLastCycle = cache.getFloatValue(view, this.mAttributeName, 0);
                if (Float.isNaN(this.mLastCycle)) {
                    this.mLastCycle = 0.0f;
                }
            }
            this.mLastCycle = (float) ((((double) this.mLastCycle) + ((delta_time * 1.0E-9d) * ((double) period))) % 1.0d);
            this.mLastTime = time;
            ...
            view.setInterpolatedValue(this.mConstraintAttributeList.valueAt(0), this.mCustomCache);
            if (period != 0.0f) {
                this.mContinue = true;
            }
    ...
    public static class CustomVarSet extends TimeCycleSplineSet {
        ...
        public boolean setProperty(MotionWidget view, float t, long time, KeyCache cache) {
            ...
            this.mConstraintAttributeList.valueAt(0).setInterpolatedValue(view, this.mCustomCache);
            if (period != 0.0f) {
                this.mContinue = true;
            }
            return this.mContinue;
