...
public class VectorDrawableCompat extends VectorDrawableCommon {
    ...
    public void draw(Canvas canvas) {
        ...
        if (!z) {
            vectorDrawableCompatState.updateCachedBitmap(scaledWidth, scaledHeight);
    ...
    private static class VPathRenderer {
        ...
        private void drawGroupTree(VGroup currentGroup, Matrix currentMatrix, Canvas canvas, int w, int h, ColorFilter filter) {
            ...
            while (i < vGroup.mChildren.size()) {
                ...
                if (child instanceof VGroup) {
                    ...
                    drawGroupTree(childGroup, vGroup.mStackedMatrix, canvas, w, h, filter);
                } else if (child instanceof VPath) {
                    VPath childPath = (VPath) child;
                    drawPath(vGroup, childPath, canvas, w, h, filter);
                }
                i++;
                vGroup = currentGroup;
            }
        ...
        private void drawPath(VGroup vGroup, VPath vPath, Canvas canvas, int w, int h, ColorFilter filter) {
            ...
            if (fullPath.mTrimPathStart != 0.0f || fullPath.mTrimPathEnd != 1.0f) {
                ...
                if (start2 > end2) {
                    pathMeasure.getSegment(start2, len2, path, true);
                    len = 0.0f;
                    this.mPathMeasure.getSegment(0.0f, end2, path, true);
                } else {
                    len = 0.0f;
                    pathMeasure.getSegment(start2, end2, path, true);
                }
