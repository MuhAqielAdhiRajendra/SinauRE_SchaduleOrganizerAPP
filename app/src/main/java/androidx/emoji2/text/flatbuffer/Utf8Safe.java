    ...
    public static String decodeUtf8Array(byte[] bytes, int index, int size) {
        ...
        while (offset < limit) {
            ...
            if (Utf8.DecodeUtil.isOneByte(byte1)) {
                ...
                Utf8.DecodeUtil.handleFourBytes(byte1, bytes[offset2], bytes[offset4], bytes[offset5], resultArr, resultPos2);
                offset = offset5 + 1;
    ...
    public static String decodeUtf8Buffer(ByteBuffer buffer, int offset, int length) {
        ...
        while (offset < limit) {
            ...
            if (Utf8.DecodeUtil.isOneByte(byte1)) {
                ...
                int offset4 = offset2 + 1;
                int offset5 = offset4 + 1;
                Utf8.DecodeUtil.handleFourBytes(byte1, buffer.get(offset2), buffer.get(offset4), buffer.get(offset5), resultArr, resultPos2);
                offset = offset5 + 1;
                resultPos2 = resultPos2 + 1 + 1;
            }
        }
