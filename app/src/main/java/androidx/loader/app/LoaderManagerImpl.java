...
class LoaderManagerImpl extends LoaderManager {
    ...
    static class LoaderViewModel extends ViewModel {
        ...
        protected void onCleared() {
            ...
            while (true) {
                ...
                if (index < size) {
                    LoaderInfo info = sparseArrayCompat.valueAt(index);
                    info.destroy(true);
                    index++;
                } else {
    ...
    private <D> Loader<D> createAndInstallLoader(int id, Bundle args, LoaderManager.LoaderCallbacks<D> callback, Loader<D> priorLoader) throws Throwable {
        try {
