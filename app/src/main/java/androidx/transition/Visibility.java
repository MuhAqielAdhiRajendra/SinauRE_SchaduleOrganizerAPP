    ...
    public Animator onDisappear(ViewGroup sceneRoot, TransitionValues startValues, int startVisibility, TransitionValues endValues, int endVisibility) {
        ...
        if (overlayView != null) {
            if (!reusingOverlayView) {
                ...
                int screenY = screenLoc[c];
                int[] loc = new int[2];
                sceneRoot.getLocationOnScreen(loc);
                overlayView.offsetLeftAndRight((screenX - loc[0]) - overlayView.getLeft());
                overlayView.offsetTopAndBottom((screenY - loc[c]) - overlayView.getTop());
        ...
        if (viewToKeep != null) {
            ...
            Animator animator3 = onDisappear(sceneRoot, viewToKeep, startValues, endValues);
            if (animator3 != null) {
                DisappearListener disappearListener = new DisappearListener(viewToKeep, endVisibility, c);
