...
public class BottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> implements MaterialBackHandler {
        ...
        this.dragCallback = new ViewDragHelper.Callback() { // from class: com.google.android.material.bottomsheet.BottomSheetBehavior.5
            ...
            public boolean tryCaptureView(View child, int pointerId) {
                ...
                if (BottomSheetBehavior.this.state == 3 && BottomSheetBehavior.this.activePointerId == pointerId) {
                    boolean z = BottomSheetBehavior.this.multipleScrollingChildrenSupported;
                    BottomSheetBehavior bottomSheetBehavior = BottomSheetBehavior.this;
                    View scroll = z ? bottomSheetBehavior.currentTouchedScrollChildRef != null ? (View) BottomSheetBehavior.this.currentTouchedScrollChildRef.get() : null : bottomSheetBehavior.getSingleScrollingChildOrNull();
                ...
                if (yvel < 0.0f) {
                    if (bottomSheetBehavior.fitToContents) {
                        ...
                        if (zShouldSkipHalfExpandedStateWhenDragging) {
                            float yPositionPercentage = (currentTop * 100.0f) / bottomSheetBehavior2.parentHeight;
                        ...
                        } else if (currentTop > bottomSheetBehavior2.halfExpandedOffset) {
                            targetState = 6;
                        } else {
                            targetState = 3;
                        }
                    }
                } else if (bottomSheetBehavior.hideable && BottomSheetBehavior.this.shouldHide(releasedChild, yvel)) {
                    ...
                    if (z) {
                        if (Math.abs(currentTop3 - bottomSheetBehavior3.fitToContentsOffset) < Math.abs(currentTop3 - BottomSheetBehavior.this.collapsedOffset)) {
                            targetState = 3;
                        } else {
                            targetState = 4;
                        }
                    } else {
                        int targetState4 = bottomSheetBehavior3.halfExpandedOffset;
                        BottomSheetBehavior bottomSheetBehavior4 = BottomSheetBehavior.this;
                        if (currentTop3 < targetState4) {
                            if (currentTop3 < Math.abs(currentTop3 - bottomSheetBehavior4.collapsedOffset)) {
                                targetState = 3;
                            ...
                            int targetState5 = bottomSheetBehavior4.halfExpandedOffset;
                            if (Math.abs(currentTop3 - targetState5) >= Math.abs(currentTop3 - BottomSheetBehavior.this.collapsedOffset) || BottomSheetBehavior.this.shouldSkipHalfExpandedStateWhenDragging()) {
                                targetState = 4;
                            } else {
                                targetState = 6;
                            }
    ...
    public BottomSheetBehavior(Context context, AttributeSet attrs) {
        ...
        this.multipleScrollingChildrenSupported = a.getBoolean(R.styleable.BottomSheetBehavior_Layout_behavior_multipleScrollingChildrenSupported, false);
        this.autoExpandOnRequestChildRectangleOffscreen = a.getBoolean(R.styleable.BottomSheetBehavior_Layout_behavior_autoExpandOnRequestChildRectangleOffscreen, true);
    ...
    public boolean onRequestChildRectangleOnScreen(CoordinatorLayout coordinatorLayout, V child, Rect rectangle, boolean immediate) {
        if (!this.autoExpandOnRequestChildRectangleOffscreen || child.isInTouchMode()) {
            return false;
        }
        if (this.state != 4 && this.state != 6) {
            return false;
        }
        if (child.getLocalVisibleRect(this.sheetRect)) {
            WindowInsetsCompat insets = ViewCompat.getRootWindowInsets(child);
            if (insets != null) {
                this.sheetRect.bottom -= insets.getInsets(WindowInsetsCompat.Type.systemBars()).bottom;
            }
            if (rectangle.top >= this.sheetRect.top && rectangle.bottom <= this.sheetRect.bottom) {
                return false;
            }
        }
        setState(3);
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (this.parentHeight - this.childHeight < this.insetTop) {
            if (this.paddingTopSystemWindowInsets) {
                ...
                if (i != -1) {
                    iMin = Math.min(iMin, this.maxHeight);
                }
        ...
        if (this.multipleScrollingChildrenSupported) {
            ...
            this.nestedScrollingChildrenRef.add(new WeakReference<>(findScrollingChild(child)));
        }
        for (int i2 = 0; i2 < this.callbacks.size(); i2++) {
            this.callbacks.get(i2).onLayout(child);
        }
        return true;
    }
    ...
    public boolean onInterceptTouchEvent(CoordinatorLayout parent, V child, MotionEvent event) {
        ...
        switch (action) {
            case 0:
                ...
                this.currentTouchedScrollChildRef = new WeakReference<>(findScrollingChildContainingCoordinates(parent, (int) event.getX(), (int) event.getY()));
                if (this.state != 2 && this.currentTouchedScrollChildRef.get() != null) {
        ...
        if (this.ignoreEvents || this.viewDragHelper == null || !this.viewDragHelper.shouldInterceptTouchEvent(event)) {
            return (action != 2 || !hasScrollingChild() || this.ignoreEvents || this.state == 1 || isTouchingScrollingChild(parent, event) || this.viewDragHelper == null || this.initialY == -1 || Math.abs(((float) this.initialY) - event.getY()) <= ((float) this.viewDragHelper.getTouchSlop())) ? false : true;
        }
        return true;
    }

    private boolean hasScrollingChild() {
        for (WeakReference<View> ref : this.nestedScrollingChildrenRef) {
            if (ref.get() != null) {
                return true;
            }
        }
    ...
    private boolean isTouchingScrollingChild(CoordinatorLayout parent, MotionEvent event) {
        if (this.multipleScrollingChildrenSupported) {
            return (this.currentTouchedScrollChildRef == null || this.currentTouchedScrollChildRef.get() == null) ? false : true;
        }
        View scroll = getSingleScrollingChildOrNull();
        return scroll != null && parent.isPointInChildBounds(scroll, (int) event.getX(), (int) event.getY());
    }
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, V child, View target, int dx, int dy, int[] consumed, int type) {
        ...
        if (dy > 0) {
            if (!this.nestedScrolled && !this.draggableOnNestedScroll && isTargetScrollingChild && target.canScrollVertically(1)) {
            ...
            if (!this.nestedScrolled && !this.draggableOnNestedScroll && isTargetScrollingChild && canScrollUp) {
                this.draggableOnNestedScrollLastDragIgnored = true;
                return;
            }
            if (!canScrollUp) {
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, V child, View target, int type) {
        ...
        if (isNestedScrollingCheckEnabled() && (!isViewScrollingChild(target) || !this.nestedScrolled)) {
            return;
        }
    ...
    public boolean onNestedPreFling(CoordinatorLayout coordinatorLayout, V child, View target, float velocityX, float velocityY) {
        if (isNestedScrollingCheckEnabled() && hasScrollingChild() && isViewScrollingChild(target)) {
            return !(this.state == 3 || this.draggableOnNestedScrollLastDragIgnored) || super.onNestedPreFling(coordinatorLayout, child, target, velocityX, velocityY);
        }
        return false;
    }
    ...
    public final void setPeekHeight(int peekHeight, boolean animate) {
        ...
        } else if (z || this.peekHeight != peekHeight) {
    ...
    public void setAutoExpandOnRequestChildRectangleOffscreen(boolean autoExpandOnRequestChildRectangleOffscreen) {
    ...
    private View findScrollingChildContainingCoordinates(CoordinatorLayout parent, int xCoordinate, int yCoordinate) {
        if (this.nestedScrollingChildrenRef.isEmpty()) {
            return null;
        }
        for (WeakReference<View> ref : this.nestedScrollingChildrenRef) {
            View scrollingChild = ref.get();
            if (scrollingChild != null && parent.isPointInChildBounds(scrollingChild, xCoordinate, yCoordinate)) {
    ...
    private boolean isTouchingDragHandle(CoordinatorLayout parent, int xCoordinate, int yCoordinate) {
        View dragHandleView = this.dragHandleViewRef != null ? this.dragHandleViewRef.get() : null;
    ...
    private void calculateCollapsedOffset() {
        ...
        if (z) {
            this.collapsedOffset = Math.max(i - peek, this.fitToContentsOffset);
        } else {
            this.collapsedOffset = i - peek;
        }
    }
    ...
    public void handleBackInvoked() {
        ...
        if (z) {
            materialBottomContainerBackHelper.finishBackProgressNotPersistent(backEvent, new AnimatorListenerAdapter() { // from class: com.google.android.material.bottomsheet.BottomSheetBehavior.2
            ...
            materialBottomContainerBackHelper.finishBackProgressPersistent(backEvent, null);
            setState(4);
        }
    }
        ...
        ViewUtils.doOnApplyWindowInsets(child, new ViewUtils.OnApplyWindowInsetsListener() { // from class: com.google.android.material.bottomsheet.BottomSheetBehavior.4
            ...
            public WindowInsetsCompat onApplyWindowInsets(View view, WindowInsetsCompat insets, ViewUtils.RelativePadding initialPadding) {
                ...
                if (BottomSheetBehavior.this.paddingLeftSystemWindowInsets) {
                    ...
                    int leftPadding3 = systemBarInsets.left;
                    leftPadding = leftPadding3 + leftPadding2;
                }
                if (BottomSheetBehavior.this.paddingRightSystemWindowInsets) {
                    int rightPadding2 = isRtl ? initialPadding.start : initialPadding.end;
                    int rightPadding3 = systemBarInsets.right;
                    rightPadding = rightPadding3 + rightPadding2;
                }
    ...
    */
    public void startSettling(android.view.View r5, int r6, boolean r7) {
        /*
            ...
            if (r7 == 0) goto L18
            int r3 = r5.getLeft()
            boolean r1 = r1.settleCapturedViewAt(r3, r0)
            if (r1 == 0) goto L25
            goto L23
        L18:
            int r3 = r5.getLeft()
            boolean r1 = r1.smoothSlideViewTo(r5, r3, r0)
            if (r1 == 0) goto L25
        L23:
            r1 = r2
            goto L26
        ...
        L26:
            ...
            r4.updateDrawableForTargetState(r6, r2)
            com.google.android.material.bottomsheet.BottomSheetBehavior<V>$StateSettlingTracker r2 = r4.stateSettlingTracker
            r2.continueSettlingToState(r6)
            goto L38
        L35:
    ...
    void setDragHandleView(BottomSheetDragHandleView dragHandleView) {
    ...
    private void updateAccessibilityActions(View view, int viewIndex) {
        ...
        switch (this.state) {
            case 3:
                if (canCollapse()) {
                    this.collapseActionIds.put(viewIndex, addAccessibilityActionForState(view, R.string.bottomsheet_action_collapse, 4));
                }
                break;
            case 4:
                this.expandActionIds.put(viewIndex, addAccessibilityActionForState(view, R.string.bottomsheet_action_expand, 3));
                break;
            case 6:
                if (canCollapse()) {
                    this.collapseActionIds.put(viewIndex, addAccessibilityActionForState(view, R.string.bottomsheet_action_collapse, 4));
    ...
    private void clearAccessibilityAction(View view, int viewIndex) {
        ...
        int expandActionId = this.expandActionIds.get(viewIndex, -1);
        if (expandActionId != -1) {
            ViewCompat.removeAccessibilityAction(view, expandActionId);
            this.expandActionIds.delete(viewIndex);
        }
        ...
        int collapseActionId = this.collapseActionIds.get(viewIndex, -1);
        if (collapseActionId != -1) {
            ViewCompat.removeAccessibilityAction(view, collapseActionId);
            this.collapseActionIds.delete(viewIndex);
        }
    }
    ...
    public static Protection getDefaultBottomGradientProtection(Context context) {
        ...
        if (color == null) {
            color = Integer.valueOf(MaterialColors.getColor(context, R.attr.colorSurface, BottomSheetBehavior.class.getSimpleName()));
        }
        return new GradientProtection(8, color.intValue());
    }
}
