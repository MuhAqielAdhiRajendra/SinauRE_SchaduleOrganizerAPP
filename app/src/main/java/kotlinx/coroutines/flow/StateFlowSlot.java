...
final class StateFlowSlot extends AbstractSharedFlowSlot<StateFlowImpl<?>> {
    ...
    public final void makePending() {
        ...
        while (true) {
            ...
            if (state == symbol) {
                if (MutatorMutex$$ExternalSyntheticBackportWithForwarding0.m(atomicReference2, state, StateFlowKt.PENDING)) {
                    return;
                }
            } else if (MutatorMutex$$ExternalSyntheticBackportWithForwarding0.m(atomicReference2, state, StateFlowKt.NONE)) {
                Result.Companion companion = Result.INSTANCE;
                ((CancellableContinuationImpl) state).resumeWith(Result.m8929constructorimpl(Unit.INSTANCE));
                return;
            }
        }
    }
    ...
    public final Object awaitPending(Continuation<? super Unit> continuation) {
        ...
        if (!MutatorMutex$$ExternalSyntheticBackportWithForwarding0.m(this._state, StateFlowKt.NONE, cont)) {
