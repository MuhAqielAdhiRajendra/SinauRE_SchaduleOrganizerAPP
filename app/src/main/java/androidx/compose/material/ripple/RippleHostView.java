...
public final class RippleHostView extends View {
    ...
    public final void m2129addRippleKOepWvA(PressInteraction.Press interaction, boolean bounded, long size, int radius, long color, float alpha, Function0<Unit> onInvalidateRipple) {
        ...
        m2130setRipplePropertiesbiQXAtU(size, radius, color, alpha);
        if (bounded) {
            ripple.setHotspot(Offset.m5068getXimpl(interaction.getPressPosition()), Offset.m5069getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m2130setRipplePropertiesbiQXAtU(long size, int radius, long color, float alpha) {
        UnprojectedRipple ripple = this.ripple;
        if (ripple == null) {
            return;
        }
        if (ripple.getRadius() != radius) {
            ripple.setRadius(radius);
        }
        ripple.m2141setColorDxMtmZc(color, alpha);
        Rect newBounds = new Rect(0, 0, MathKt.roundToInt(Size.m5137getWidthimpl(size)), MathKt.roundToInt(Size.m5134getHeightimpl(size)));
    ...
    static final void setRippleState$lambda$1(RippleHostView this$0) {
