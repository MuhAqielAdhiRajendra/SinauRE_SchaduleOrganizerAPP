...
public final class DispatchedTaskKt {
    ...
    public static final <T> void dispatch(DispatchedTask<? super T> dispatchedTask, int mode) {
        ...
        if (!undispatched && (delegate$kotlinx_coroutines_core instanceof DispatchedContinuation) && isCancellableMode(mode) == isCancellableMode(dispatchedTask.resumeMode)) {
            CoroutineDispatcher dispatcher = ((DispatchedContinuation) delegate$kotlinx_coroutines_core).dispatcher;
            CoroutineContext context = ((DispatchedContinuation) delegate$kotlinx_coroutines_core).get$context();
            if (dispatcher.isDispatchNeeded(context)) {
                dispatcher.mo10498dispatch(context, dispatchedTask);
                return;
            } else {
        ...
        resume(dispatchedTask, delegate$kotlinx_coroutines_core, undispatched);
    }
    ...
    public static final <T> void resume(DispatchedTask<? super T> dispatchedTask, Continuation<? super T> continuation, boolean undispatched) {
        ...
        Object result = Result.m8929constructorimpl(exception != null ? ResultKt.createFailure(exception) : dispatchedTask.getSuccessfulResult$kotlinx_coroutines_core(state));
        if (!undispatched) {
        ...
        CoroutineContext context$iv$iv = continuation2.get$context();
        ...
        if (oldValue$iv$iv != ThreadContextKt.NO_THREAD_ELEMENTS) {
            undispatchedCoroutineUpdateUndispatchedCompletion = CoroutineContextKt.updateUndispatchedCompletion(continuation2, context$iv$iv, oldValue$iv$iv);
        } else {
    ...
    public static final void resumeWithStackTrace(Continuation<?> continuation, Throwable exception) {
        ...
        if (DebugKt.getRECOVER_STACK_TRACES() && (continuation instanceof CoroutineStackFrame)) {
            thRecoverFromStackFrame = StackTraceRecoveryKt.recoverFromStackFrame(exception, (CoroutineStackFrame) continuation);
        } else {
            thRecoverFromStackFrame = exception;
        }
        continuation.resumeWith(Result.m8929constructorimpl(ResultKt.createFailure(thRecoverFromStackFrame)));
    }
}
