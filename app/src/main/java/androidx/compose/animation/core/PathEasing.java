    ...
    private final void initializeEasing() {
        ...
        while (true) {
            ...
            if (segment.getType() != PathSegment.Type.Move && segment.getType() != PathSegment.Type.Done) {
                ...
                intervalTree.addInterval(Float.intBitsToFloat(bits$iv$iv), Float.intBitsToFloat(bits$iv$iv2), segment);
            }
        }
        value$iv = intervalTree.contains(0.0f) && intervalTree.contains(1.0f);
        if (!value$iv) {
