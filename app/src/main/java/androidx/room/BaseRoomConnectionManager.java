...
public abstract class BaseRoomConnectionManager {
    ...
    protected final class DriverWrapper implements SQLiteDriver {
        ...
        static final SQLiteConnection openLocked$lambda$1(BaseRoomConnectionManager this$0, DriverWrapper this$1, String $filename) throws Exception {
    ...
    public final void configureDatabase(SQLiteConnection connection) throws Exception {
        ...
        try {
            SQLiteStatement statement = sQLiteStatementPrepare;
            statement.step();
            ...
            if (version != getOpenDelegate().getVersion()) {
                ...
                try {
                    ...
                    objM8929constructorimpl = Result.m8929constructorimpl(Unit.INSTANCE);
                } catch (Throwable th) {
                    Result.Companion companion2 = Result.INSTANCE;
                    objM8929constructorimpl = Result.m8929constructorimpl(ResultKt.createFailure(th));
                }
                if (Result.m8936isSuccessimpl(objM8929constructorimpl)) {
                    SQLite.execSQL(connection, "END TRANSACTION");
                }
                Throwable it = Result.m8932exceptionOrNullimpl(objM8929constructorimpl);
                if (it != null) {
    ...
    protected final void onMigrate(SQLiteConnection connection, int oldVersion, int newVersion) throws Exception {
        ...
        Iterable iterableFindMigrationPath = MigrationUtil.findMigrationPath(getConfiguration().migrationContainer, oldVersion, newVersion);
        if (iterableFindMigrationPath != null) {
    ...
    private final void dropAllTables(SQLiteConnection connection) throws Exception {
        if (getConfiguration().allowDestructiveMigrationForAllTables) {
            ...
            try {
                ...
                Iterable iterableBuild = CollectionsKt.build($this$dropAllTables_u24lambda_u248_u24lambda_u247);
                AutoCloseableKt.closeFinally(sQLiteStatementPrepare, null);
    ...
    private final void checkIdentity(SQLiteConnection connection) throws Exception {
        ...
        try {
            ...
            Result.Companion companion2 = Result.INSTANCE;
            objM8929constructorimpl = Result.m8929constructorimpl(ResultKt.createFailure(th3));
        }
        if (!result.isValid) {
            throw new IllegalStateException(("Pre-packaged database has an invalid schema: " + result.expectedFoundMsg).toString());
        }
        ...
        Throwable it2 = Result.m8932exceptionOrNullimpl(objM8929constructorimpl);
        if (it2 == null) {
            Result.m8928boximpl(objM8929constructorimpl);
        } else {
            SQLite.execSQL(connection, "ROLLBACK TRANSACTION");
            throw it2;
        }
