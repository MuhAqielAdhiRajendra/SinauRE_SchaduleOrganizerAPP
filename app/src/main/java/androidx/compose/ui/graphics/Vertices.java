...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m5367toArgb8_81llA(colors.get(i).m5323unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            int pointIndex = i / 2;
            long point = points.get(pointIndex).m5078unboximpl();
            if (i % 2 == 0) {
