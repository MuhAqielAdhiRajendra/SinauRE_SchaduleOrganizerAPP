...
public final class ArrayTypeAdapter<E> extends TypeAdapter<Object> {
    public static final TypeAdapterFactory FACTORY = new TypeAdapterFactory() { // from class: com.google.gson.internal.bind.ArrayTypeAdapter.1
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> typeToken) {
            ...
            TypeAdapter<T> arrayAdapter = new ArrayTypeAdapter<>(gson, gson.getAdapter(TypeToken.get(componentType)), GsonTypes.getRawType(componentType));
            return arrayAdapter;
        }
    };
    ...
    public Object read(JsonReader in) throws IOException {
        ...
        if (zIsPrimitive) {
            Object array = Array.newInstance((Class<?>) cls, size);
            for (int i = 0; i < size; i++) {
                Array.set(array, i, list.get(i));
            }
            return array;
        }
        return list.toArray((Object[]) Array.newInstance((Class<?>) cls, size));
    }
