...
public class KeyPosition extends KeyPositionBase {
    ...
    private void calcScreenPosition(int layoutWidth, int layoutHeight) {
        this.mCalculatedPositionX = ((layoutWidth - 0) * this.mPercentX) + (0 / 2);
        this.mCalculatedPositionY = ((layoutHeight - 0) * this.mPercentX) + (0 / 2);
    }
    ...
    private void calcPathPosition(float startX, float startY, float endX, float endY) {
        ...
        this.mCalculatedPositionX = (this.mPercentX * pathVectorX) + startX + (this.mPercentY * perpendicularX);
    ...
    private void calcCartesianPosition(float startX, float startY, float endX, float endY) {
        ...
        this.mCalculatedPositionX = (int) ((pathVectorX * dxdx) + startX + (pathVectorY * dxdy));
        this.mCalculatedPositionY = (int) ((pathVectorX * dydx) + startY + (pathVectorY * dydy));
    }
    ...
    void positionAxisAttributes(RectF start, RectF end, float x, float y, String[] attribute, float[] value) {
        float startCenterX = start.centerX();
        float startCenterY = start.centerY();
        float endCenterX = end.centerX();
        float endCenterY = end.centerY();
        ...
        if (attribute[0] != null) {
            if ("percentX".equals(attribute[0])) {
                value[0] = (x - startCenterX) / tmp;
                value[1] = (y - startCenterY) / pathVectorY;
                return;
            } else {
                value[1] = (x - startCenterX) / tmp;
                value[0] = (y - startCenterY) / pathVectorY;
                return;
            }
        }
