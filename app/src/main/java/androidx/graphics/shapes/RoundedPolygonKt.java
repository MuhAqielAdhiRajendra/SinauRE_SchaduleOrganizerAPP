...
public final class RoundedPolygonKt {
    ...
    */
    ...
    private static final long calculateCenter(float[] vertices) {
        ...
        return FloatFloatPair.m10constructorimpl((cumulativeX / vertices.length) / 2.0f, (cumulativeY / vertices.length) / 2.0f);
    }
    ...
    private static final float[] verticesFromNumVerts(int numVertices, float radius, float centerX, float centerY) {
        ...
        while (i < numVertices) {
            ...
            long vertex = PointKt.m8479plusybeJwSQ(Utils.m8495radialToCartesianL6JJ3z0$default(radius2, (Utils.getFloatPi() / numVertices) * 2.0f * i, 0L, 4, null), FloatFloatPair.m10constructorimpl(centerX, centerY));
            int arrayIndex2 = arrayIndex + 1;
            result[arrayIndex] = PointKt.m8475getXDnnuFBc(vertex);
            arrayIndex = arrayIndex2 + 1;
            result[arrayIndex2] = PointKt.m8476getYDnnuFBc(vertex);
            i++;
            radius = radius2;
        }
        return result;
