...
class MonthAdapter extends BaseAdapter {
    ...
    private void updateSelectedState(TextView dayTextView, long date, int dayNumber) {
        ...
        if (valid) {
            ...
            if (selected2) {
                ...
                boolean zIsToday = isToday(date);
                CalendarStyle calendarStyle = this.calendarStyle;
                if (zIsToday) {
                    selected = selected2;
                    style = calendarStyle.todayDay;
                } else {
                    CalendarItemStyle style2 = calendarStyle.day;
                    selected = selected2;
                    style = style2;
                }
            }
    ...
    boolean isDayPositionValid(int position) {
        ...
        return day != null && this.calendarConstraints.getDateValidator().isValid(day.longValue());
    }
    ...
    int findNearestValidDayPositionInRow(int position) {
        ...
        for (int i = 1; i < this.month.daysInWeek; i++) {
            int rightPosition = position + i;
            if (rightPosition < getCount() && getItemId(rightPosition) == rowId && isDayPositionValid(rightPosition)) {
                return rightPosition;
            }
            int leftPosition = position - i;
            if (leftPosition >= 0 && getItemId(leftPosition) == rowId && isDayPositionValid(leftPosition)) {
                return leftPosition;
            }
        }
        return -1;
