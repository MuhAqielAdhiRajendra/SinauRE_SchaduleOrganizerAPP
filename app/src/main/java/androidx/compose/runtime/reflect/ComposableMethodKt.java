...
/* JADX INFO: compiled from: ComposableMethod.jvmAndAndroid.kt */
...
public final class ComposableMethodKt {
    ...
    private static final ComposableInfo getComposableInfo(Method method) {
        ...
        if (length >= 0) {
            do {
                i = length;
                length--;
                if (Intrinsics.areEqual(parameterTypes[i], Composer.class)) {
                    break;
                }
            } while (length >= 0);
            i = -1;
        ...
        int iChangedParamCount = changedParamCount(i, !Modifier.isStatic(method.getModifiers()) ? 1 : 0);
        int i2 = i + 1 + iChangedParamCount;
        int length2 = method.getParameterTypes().length;
        int iDefaultParamCount = (length2 != i2) != false ? defaultParamCount(i) : 0;
        return new ComposableInfo(i2 + iDefaultParamCount == length2, i, iChangedParamCount, iDefaultParamCount);
    }
    ...
    public static final ComposableMethod getDeclaredComposableMethod(Class<?> cls, String methodName, Class<?>... clsArr) throws NoSuchMethodException {
        ...
        ComposableMethod composableMethodAsComposableMethod = asComposableMethod(method);
