...
final class MaterialCalendarGridView extends GridView {
    ...
    protected void onAttachedToWindow() {
        ...
        post(new Runnable() { // from class: com.google.android.material.datepicker.MaterialCalendarGridView$$ExternalSyntheticLambda0
            ...
            public final void run() {
                this.f$0.m8840x267001b4();
            }
        });
    }

    /* JADX INFO: renamed from: lambda$onAttachedToWindow$0$com-google-android-material-datepicker-MaterialCalendarGridView, reason: not valid java name */
    /* synthetic */ void m8840x267001b4() {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            ...
            default:
                ...
                if (selectedPosition != -1 && !adapter.isDayPositionValid(selectedPosition)) {
    ...
    boolean handleVerticalNavigationOnDisabledDay(int keyCode, int selectedPosition) {
        ...
        if (keyCode == 20) {
            int nextPositionInColumn = getNumColumns() + selectedPosition;
    ...
    private boolean handleHorizontalNavigation(int position, boolean forward) {
        int nextPosition;
        if (forward) {
            nextPosition = getAdapter().findNextValidDayPosition(position);
        } else {
            nextPosition = getAdapter().findPreviousValidDayPosition(position);
        }
        if (nextPosition != -1) {
            setSelection(nextPosition);
            return true;
        }
        if (!forward && this.onMonthNavigationListener != null) {
            return this.onMonthNavigationListener.onMonthNavigationPrevious();
        }
        if (!forward || this.onMonthNavigationListener == null) {
            return true;
        }
        return this.onMonthNavigationListener.onMonthNavigationNext();
    }
                ...
                if (!skipMonth(firstOfMonth, lastOfMonth, Long.valueOf(startItem), Long.valueOf(endItem))) {
                    ...
                    while (bottom <= lastRow) {
                        ...
                        if (!isRtl) {
                            ...
                            width = rangeHighlightStart3 > lastRow3 ? getWidth() : rangeHighlightEnd;
                        } else {
