...
public class ShadowDrawableWrapper extends DrawableWrapperCompat {
    ...
    private void drawShadow(Canvas canvas) {
        ...
        boolean drawVerticalEdges = this.contentBounds.height() - (shadowOffset3 * 2.0f) > 0.0f;
        ...
        float shadowScaleHorizontal2 = shadowOffset3 / (shadowOffset3 + shadowOffsetHorizontal);
        float shadowScaleTop2 = shadowOffset3 / (shadowOffset3 + shadowOffsetTop);
        float shadowScaleBottom = shadowOffset3 / (shadowOffset3 + shadowOffsetBottom);
        int saved2 = canvas.save();
        canvas.translate(this.contentBounds.left + shadowOffset3, this.contentBounds.top + shadowOffset3);
        canvas.scale(shadowScaleHorizontal2, shadowScaleTop2);
        ...
        if (!drawHorizontalEdges2) {
            ...
            canvas.scale(1.0f / shadowScaleHorizontal2, 1.0f);
            shadowScaleTop = shadowScaleTop2;
            shadowScaleHorizontal = shadowScaleHorizontal2;
            drawHorizontalEdges = drawHorizontalEdges2;
            saved = saved2;
            shadowOffset = shadowOffset3;
            shadowOffset2 = shadowScaleBottom;
            canvas.drawRect(0.0f, edgeShadowTop, this.contentBounds.width() - (shadowOffset3 * 2.0f), -this.cornerRadius, this.edgeShadowPaint);
        }
        canvas.restoreToCount(saved);
        int saved3 = canvas.save();
        canvas.translate(this.contentBounds.right - shadowOffset, this.contentBounds.bottom - shadowOffset);
        canvas.scale(shadowScaleHorizontal, shadowOffset2);
        canvas.rotate(180.0f);
        canvas.drawPath(this.cornerShadowPath, this.cornerShadowPaint);
        if (drawHorizontalEdges) {
            canvas.scale(1.0f / shadowScaleHorizontal, 1.0f);
            canvas.drawRect(0.0f, edgeShadowTop, this.contentBounds.width() - (shadowOffset * 2.0f), this.shadowSize + (-this.cornerRadius), this.edgeShadowPaint);
        }
        ...
        if (drawVerticalEdges) {
            canvas.scale(1.0f / shadowOffset2, 1.0f);
