...
public final class AndroidImageBitmap implements ImageBitmap {
    ...
    public int mo5177getConfig_sVssgQ() {
    ...
    public void readPixels(int[] buffer, int startX, int startY, int width, int height, int bufferOffset, int stride) {
        ...
        if (Build.VERSION.SDK_INT < 26 || androidBitmap.getConfig() != Bitmap.Config.HARDWARE) {
            bitmapCopy = androidBitmap;
        } else {
            recycleTarget = true;
            bitmapCopy = androidBitmap.copy(Bitmap.Config.ARGB_8888, false);
        }
        Bitmap targetBitmap = bitmapCopy;
