...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler, RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (state.isPreLayout() && this.mPendingScrollPosition != -1 && this.mPendingScrollPositionOffset != Integer.MIN_VALUE && (existing = findViewByPosition(this.mPendingScrollPosition)) != null) {
            ...
            if (z) {
                int current = orientationHelper.getEndAfterPadding() - this.mOrientationHelper.getDecoratedEnd(existing);
                upcomingOffset = current - this.mPendingScrollPositionOffset;
            } else {
                int current2 = orientationHelper.getDecoratedStart(existing);
                upcomingOffset = this.mPendingScrollPositionOffset - (current2 - this.mOrientationHelper.getStartAfterPadding());
            }
            if (upcomingOffset > 0) {
        ...
        boolean z4 = this.mAnchorInfo.mLayoutFromEnd;
    ...
    private boolean updateAnchorFromChildren(RecyclerView.Recycler recycler, RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        if (this.mLastStackFromEnd != this.mStackFromEnd || (referenceChild = findReferenceChild(recycler, state, anchorInfo.mLayoutFromEnd, this.mStackFromEnd)) == null) {
            return false;
        }
    ...
    private boolean updateAnchorFromPendingData(RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        if (this.mPendingScrollPositionOffset == Integer.MIN_VALUE) {
            ...
            if (child != null) {
                ...
                int endGap = orientationHelper2.getEndAfterPadding() - this.mOrientationHelper.getDecoratedEnd(child);
                if (endGap < 0) {
                ...
                if (z2) {
                    decoratedStart = orientationHelper3.getDecoratedEnd(child) + this.mOrientationHelper.getTotalSpaceChange();
                } else {
        ...
        if (z3) {
            anchorInfo.mCoordinate = orientationHelper4.getEndAfterPadding() - this.mPendingScrollPositionOffset;
        } else {
            anchorInfo.mCoordinate = orientationHelper4.getStartAfterPadding() + this.mPendingScrollPositionOffset;
        }
        return true;
    }
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LayoutState layoutState, LayoutChunkResult result) {
        ...
        List<RecyclerView.ViewHolder> list = layoutState.mScrapList;
        boolean z = this.mShouldReverseLayout;
        if (list == null) {
            if (z == (layoutState.mLayoutDirection == -1)) {
                addView(view);
            } else {
                addView(view, 0);
            }
        ...
        if (this.mOrientation == 1) {
            ...
            if (layoutState.mLayoutDirection == -1) {
                int right2 = layoutState.mOffset;
                top = right2;
        ...
        layoutDecoratedWithMargins(view, left, bottom, top, bottom2);
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount) {
                if (((RecyclerView.LayoutParams) view.getLayoutParams()).isItemRemoved()) {
                    ...
                    boolean outOfBoundsAfter = false;
                    if (childEnd <= boundsStart && childStart < boundsStart) {
                        outOfBoundsBefore = true;
                    } else {
                        outOfBoundsBefore = false;
                    }
                    if (childStart >= boundsEnd && childEnd > boundsEnd) {
                        outOfBoundsAfter = true;
                    }
    ...
    public void prepareForDrop(View view, View target, int x, int y) {
        ...
        if (this.mShouldReverseLayout) {
            OrientationHelper orientationHelper = this.mOrientationHelper;
            if (dropDirection == 1) {
                scrollToPositionWithOffset(targetPos, orientationHelper.getEndAfterPadding() - (this.mOrientationHelper.getDecoratedStart(target) + this.mOrientationHelper.getDecoratedMeasurement(view)));
                return;
            } else {
                scrollToPositionWithOffset(targetPos, orientationHelper.getEndAfterPadding() - this.mOrientationHelper.getDecoratedEnd(target));
                return;
            }
        }
        OrientationHelper orientationHelper2 = this.mOrientationHelper;
        if (dropDirection == -1) {
            scrollToPositionWithOffset(targetPos, orientationHelper2.getDecoratedStart(target));
        } else {
            scrollToPositionWithOffset(targetPos, orientationHelper2.getDecoratedEnd(target) - this.mOrientationHelper.getDecoratedMeasurement(view));
        }
    }
    ...
    static class AnchorInfo {
        ...
        public void assignFromViewAndKeepVisibleRect(View child, int position) {
            ...
            OrientationHelper orientationHelper = this.mOrientationHelper;
            if (z) {
                int prevLayoutEnd = orientationHelper.getEndAfterPadding() - spaceChange;
        ...
        public void assignFromView(View child, int position) {
            ...
            OrientationHelper orientationHelper = this.mOrientationHelper;
            if (z) {
                this.mCoordinate = orientationHelper.getDecoratedEnd(child) + this.mOrientationHelper.getTotalSpaceChange();
            } else {
                this.mCoordinate = orientationHelper.getDecoratedStart(child);
            }
            this.mPosition = position;
        }
    }
