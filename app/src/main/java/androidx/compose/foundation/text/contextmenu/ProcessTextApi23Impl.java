...
public final class ProcessTextApi23Impl {
    ...
    static final List processTextActivitiesQuery$lambda$0(Context context) {
        ...
        List target$iv = new ArrayList(listQueryIntentActivities.size());
        int size = listQueryIntentActivities.size();
        for (int index$iv$iv = 0; index$iv$iv < size; index$iv$iv++) {
            ResolveInfo resolveInfo = listQueryIntentActivities.get(index$iv$iv);
    ...
    static final Unit onClickProcessTextItem$lambda$0(Context context, ResolveInfo resolveInfo, boolean editable, CharSequence text, TextRange selection) {
        String selectedText = text.subSequence(TextRange.m7571getMinimpl(selection.getPackedValue()), TextRange.m7570getMaximpl(selection.getPackedValue())).toString();
