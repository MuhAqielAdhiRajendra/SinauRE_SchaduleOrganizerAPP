...
class IconHelper {
    ...
    private static void setIconClickable(CheckableImageButton iconView, View.OnLongClickListener onLongClickListener) {
        ...
        if (Build.VERSION.SDK_INT >= 26 || !iconFocusable || iconLongClickable) {
            iconView.setLongClickable(iconLongClickable);
        }
        iconView.setImportantForAccessibility(iconFocusable ? 1 : 2);
    }
    ...
    static void updateIconTooltip(CheckableImageButton iconView, View.OnLongClickListener onLongClickListener, CharSequence tooltip) {
        CharSequence tooltipText = iconView.isFocusable() ? tooltip : null;
