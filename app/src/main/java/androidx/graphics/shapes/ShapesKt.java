...
public final class ShapesKt {
    ...
    public static final RoundedPolygon rectangle(RoundedPolygon.Companion $this$rectangle, float width, float height, CornerRounding rounding, List<CornerRounding> list, float centerX, float centerY) {
        ...
        float right = (width / 2.0f) + centerX;
        float bottom = (height / 2.0f) + centerY;
    ...
    private static final float[] pillStarVerticesFromNumVerts(int numVerticesPerRadius, float width, float height, float innerRadius, float vertexSpacing, float startLocation, float centerX, float centerY) {
        ...
        float vSegHalf = vSegLen / 2.0f;
        float hSegHalf = hSegLen / 2.0f;
        float circlePerimeter = Utils.getTwoPi() * endcapRadius * Utils.interpolate(innerRadius, 1.0f, vertexSpacing);
        float perimeter = (2.0f * hSegLen) + (2.0f * vSegLen) + circlePerimeter;
        float[] sections = new float[11];
        sections[0] = 0.0f;
        sections[1] = vSegLen / 2.0f;
        sections[2] = sections[1] + (circlePerimeter / 4.0f);
        sections[3] = sections[2] + hSegLen;
        sections[4] = sections[3] + (circlePerimeter / 4.0f);
        sections[5] = sections[4] + vSegLen;
        sections[6] = sections[5] + (circlePerimeter / 4.0f);
        sections[7] = sections[6] + hSegLen;
        sections[8] = sections[7] + (circlePerimeter / 4.0f);
        sections[9] = sections[8] + (vSegLen / 2.0f);
        ...
        int arrayIndex = 0;
        long rectBR = FloatFloatPair.m10constructorimpl(hSegHalf, vSegHalf);
        long rectBL = FloatFloatPair.m10constructorimpl(-hSegHalf, vSegHalf);
        float tPerVertex2 = -vSegHalf;
        long rectTL = FloatFloatPair.m10constructorimpl(-hSegHalf, tPerVertex2);
        long rectTR = FloatFloatPair.m10constructorimpl(hSegHalf, -vSegHalf);
            ...
            while (true) {
                ...
                if (boundedT >= sections[(currSecIndex + 1) % sections.length]) {
                    ...
                    switch (currSecIndex) {
                        case 0:
                            i = i3;
                            float tInSection2 = tProportion * vSegHalf;
                            vertex = FloatFloatPair.m10constructorimpl(currRadius, tInSection2);
                            break;
                        case 1:
                            i = i3;
                            float tInSection3 = Utils.getFloatPi();
                            vertex = PointKt.m8479plusybeJwSQ(Utils.m8495radialToCartesianL6JJ3z0$default(currRadius, (tInSection3 * tProportion) / 2.0f, 0L, 4, null), rectBR);
                            break;
                        case 2:
                            i = i3;
                            float tInSection4 = tProportion * hSegLen;
                            vertex = FloatFloatPair.m10constructorimpl(hSegHalf - tInSection4, currRadius);
                            break;
                        case 3:
                            i = i3;
                            float tInSection5 = Utils.getFloatPi();
                            vertex = PointKt.m8479plusybeJwSQ(Utils.m8495radialToCartesianL6JJ3z0$default(currRadius, (tInSection5 / 2.0f) + ((Utils.getFloatPi() * tProportion) / 2.0f), 0L, 4, null), rectBL);
                            break;
                        case 4:
                            i = i3;
                            float tInSection6 = -currRadius;
                            vertex = FloatFloatPair.m10constructorimpl(tInSection6, vSegHalf - (tProportion * vSegLen));
                            break;
                        case 5:
                            i = i3;
                            float tInSection7 = Utils.getFloatPi();
                            vertex = PointKt.m8479plusybeJwSQ(Utils.m8495radialToCartesianL6JJ3z0$default(currRadius, tInSection7 + ((Utils.getFloatPi() * tProportion) / 2.0f), 0L, 4, null), rectTL);
                            break;
                        case 6:
                            i = i3;
                            float tInSection8 = -hSegHalf;
                            vertex = FloatFloatPair.m10constructorimpl(tInSection8 + (tProportion * hSegLen), -currRadius);
                            break;
                        case 7:
                            i = i3;
                            vertex = PointKt.m8479plusybeJwSQ(Utils.m8495radialToCartesianL6JJ3z0$default(currRadius, (Utils.getFloatPi() * 1.5f) + ((Utils.getFloatPi() * tProportion) / 2.0f), 0L, 4, null), rectTR);
                            break;
                        default:
                            i = i3;
                            float tInSection9 = -vSegHalf;
                            vertex = FloatFloatPair.m10constructorimpl(currRadius, tInSection9 + (tProportion * vSegHalf));
                            break;
                    }
                    int arrayIndex2 = arrayIndex + 1;
                    result[arrayIndex] = PointKt.m8475getXDnnuFBc(vertex) + centerX;
                    arrayIndex = arrayIndex2 + 1;
                    result[arrayIndex2] = PointKt.m8476getYDnnuFBc(vertex) + centerY;
                    t += tPerVertex;
                    inner = !inner;
                    i3 = i + 1;
                    i2 = i4;
    ...
    private static final float[] starVerticesFromNumVerts(int numVerticesPerRadius, float radius, float innerRadius, float centerX, float centerY) {
        ...
        for (int i = 0; i < numVerticesPerRadius; i++) {
            long vertex = Utils.m8495radialToCartesianL6JJ3z0$default(radius, (Utils.getFloatPi() / numVerticesPerRadius) * 2.0f * i, 0L, 4, null);
            int arrayIndex2 = arrayIndex + 1;
            result[arrayIndex] = PointKt.m8475getXDnnuFBc(vertex) + centerX;
            int arrayIndex3 = arrayIndex2 + 1;
            result[arrayIndex2] = PointKt.m8476getYDnnuFBc(vertex) + centerY;
            long vertex2 = Utils.m8495radialToCartesianL6JJ3z0$default(innerRadius, (Utils.getFloatPi() / numVerticesPerRadius) * ((i * 2) + 1), 0L, 4, null);
            int arrayIndex4 = arrayIndex3 + 1;
            result[arrayIndex3] = PointKt.m8475getXDnnuFBc(vertex2) + centerX;
            arrayIndex = arrayIndex4 + 1;
            result[arrayIndex4] = PointKt.m8476getYDnnuFBc(vertex2) + centerY;
        }
        return result;
    }
}
