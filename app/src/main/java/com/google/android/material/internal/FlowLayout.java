    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        while (i < getChildCount()) {
            ...
            if (child.getVisibility() == 8) {
                ...
                if (childLeft2 + leftMargin + child.getMeasuredWidth() > maxRight && !isSingleLine()) {
                    int childLeft3 = getPaddingLeft();
                    childTop = childBottom + this.lineSpacing;
                    childLeft2 = childLeft3;
                }
                int childRight = childLeft2 + leftMargin + child.getMeasuredWidth();
                int childBottom2 = child.getMeasuredHeight() + childTop;
                if (childRight > maxChildRight) {
                    maxChildRight = childRight;
                }
                int childLeft4 = childLeft2 + leftMargin + rightMargin + child.getMeasuredWidth() + this.itemSpacing;
