...
public class SemaphoreAndMutexImpl {
    ...
    public SemaphoreAndMutexImpl(int permits, int acquiredPermits) {
        ...
        this.onCancellationRelease = new Function3() { // from class: kotlinx.coroutines.sync.SemaphoreAndMutexImpl$$ExternalSyntheticLambda2
    ...
    static final Unit onCancellationRelease$lambda$2(SemaphoreAndMutexImpl this$0, Throwable th, Unit unit, CoroutineContext coroutineContext) {
        this$0.release();
    ...
    */
    ...
    public final boolean addAcquireToQueue(Waiter waiter) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m10504getSegmentimpl(s$iv);
        int i = (int) (enqIdx % ((long) SemaphoreKt.SEGMENT_SIZE));
        if (!AtomicArraysKt__AtomicArrays_jvmKt$$ExternalSyntheticBackportWithForwarding0.m(segment.getAcquirers(), i, null, waiter)) {
            Object expected$iv = SemaphoreKt.PERMIT;
            Object value$iv = SemaphoreKt.TAKEN;
            if (AtomicArraysKt__AtomicArrays_jvmKt$$ExternalSyntheticBackportWithForwarding0.m(segment.getAcquirers(), i, expected$iv, value$iv)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        while (true) {
            ...
            if (!SegmentOrClosed.m10506isClosedimpl(objFindSegmentInternal)) {
                Segment segmentM10504getSegmentimpl = SegmentOrClosed.m10504getSegmentimpl(objFindSegmentInternal);
                while (true) {
                    Segment segment = (Segment) atomicReferenceFieldUpdater.get(this);
                    z = false;
                    z2 = true;
                    if (segment.id >= segmentM10504getSegmentimpl.id) {
                        z3 = true;
                        break;
                    }
                    if (!segmentM10504getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        z3 = false;
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, this, segment, segmentM10504getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                        z3 = true;
                    } else if (segmentM10504getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM10504getSegmentimpl.remove();
                    }
                }
                if (z3) {
                    break;
        ...
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) SegmentOrClosed.m10504getSegmentimpl(objFindSegmentInternal);
        ...
        if (andSet != null) {
            return andSet == SemaphoreKt.CANCELLED ? z : tryResumeAcquire(andSet);
        }
        int i2 = SemaphoreKt.MAX_SPIN_CYCLES;
        for (?? r11 = z; r11 < i2; r11++) {
            SemaphoreSegment semaphoreSegment3 = semaphoreSegment;
            if (semaphoreSegment2.getAcquirers().get(i) == SemaphoreKt.TAKEN) {
                return z2;
            }
            semaphoreSegment = semaphoreSegment3;
        }
        return !AtomicArraysKt__AtomicArrays_jvmKt$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment2.getAcquirers(), i, SemaphoreKt.PERMIT, SemaphoreKt.BROKEN);
    }
