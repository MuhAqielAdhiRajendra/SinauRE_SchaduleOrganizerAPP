...
/* JADX INFO: compiled from: RoomConnectionManager.android.kt */
...
public final class RoomConnectionManager extends BaseRoomConnectionManager {
    ...
    public RoomConnectionManager(DatabaseConfiguration config, RoomOpenDelegate openDelegate, Function2<? super Function1<? super Continuation<Object>, ? extends Object>, ? super Continuation<Object>, ? extends Object> transactionWrapper) {
        PassthroughConnectionPool passthroughConnectionPoolNewConnectionPool;
        ...
        if (config.sqliteDriver == null) {
            ...
            if (config.sqliteDriver.hasConnectionPool()) {
                ...
                passthroughConnectionPoolNewConnectionPool = new PassthroughConnectionPool(driverWrapper, str2 != null ? str2 : ":memory:", transactionWrapper);
            } else if (config.name == null) {
                passthroughConnectionPoolNewConnectionPool = ConnectionPoolKt.newSingleConnectionPool(new BaseRoomConnectionManager.DriverWrapper(this, config.sqliteDriver), ":memory:", config.getPreparedStatementCacheSize());
            } else {
                passthroughConnectionPoolNewConnectionPool = ConnectionPoolKt.newConnectionPool(new BaseRoomConnectionManager.DriverWrapper(this, config.sqliteDriver), config.name, getMaxNumberOfReaders(config.journalMode), getMaxNumberOfWriters(config.journalMode), config.getPreparedStatementCacheSize());
            }
            this.connectionPool = passthroughConnectionPoolNewConnectionPool;
        }
        init();
    }
    ...
    static final Unit _init_$lambda$1(RoomConnectionManager this$0, SupportSQLiteDatabase db) {
    ...
    private final DatabaseConfiguration installOnOpenCallback(DatabaseConfiguration $this$installOnOpenCallback, final Function1<? super SupportSQLiteDatabase, Unit> function1) {
        List<RoomDatabase.Callback> listEmptyList = $this$installOnOpenCallback.callbacks;
        if (listEmptyList == null) {
            listEmptyList = CollectionsKt.emptyList();
        }
        List newCallbacks = CollectionsKt.plus((Collection<? extends RoomDatabase.Callback>) listEmptyList, new RoomDatabase.Callback() { // from class: androidx.room.RoomConnectionManager$installOnOpenCallback$newCallbacks$1
