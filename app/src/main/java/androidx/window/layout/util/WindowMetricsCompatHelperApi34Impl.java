...
public final class WindowMetricsCompatHelperApi34Impl implements WindowMetricsCompatHelper {
    ...
    public WindowMetrics currentWindowMetrics(Context context, DensityCompatHelper densityCompatHelper) {
        ...
        if (context.isUiContext()) {
            wm = (WindowManager) context.getSystemService(WindowManager.class);
        } else {
            wm = (WindowManager) context.getApplicationContext().getSystemService(WindowManager.class);
        }
        Rect bounds = wm.getCurrentWindowMetrics().getBounds();
        Intrinsics.checkNotNullExpressionValue(bounds, "getBounds(...)");
        return new WindowMetrics(bounds, wm.getCurrentWindowMetrics().getDensity());
    }
