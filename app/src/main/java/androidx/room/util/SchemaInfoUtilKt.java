...
public final class SchemaInfoUtilKt {
    ...
    private static final Set<TableInfo.ForeignKey> readForeignKeys(SQLiteConnection connection, String tableName) throws Exception {
        int id;
        SQLiteStatement sQLiteStatementPrepare = connection.prepare("PRAGMA foreign_key_list(`" + tableName + "`)");
        try {
            ...
            Iterable foreignKeyFieldMappings = readForeignKeyFieldMappings(stmt);
            stmt.reset();
            ...
            while (stmt.step()) {
                ...
                if (seq == 0) {
                    ...
                    Iterable $this$filter$iv = foreignKeyFieldMappings;
    ...
    private static final Map<String, TableInfo.Column> readColumns(SQLiteConnection connection, String tableName) throws Exception {
        ...
        try {
            ...
            Map<String, TableInfo.Column> mapBuild = MapsKt.build($this$readColumns_u24lambda_u246_u24lambda_u245);
            AutoCloseableKt.closeFinally(sQLiteStatementPrepare, null);
            return mapBuild;
        } catch (Throwable th) {
            try {
    ...
    private static final Set<TableInfo.Index> readIndices(SQLiteConnection connection, String tableName) throws Exception {
        SQLiteStatement sQLiteStatementPrepare = connection.prepare("PRAGMA index_list(`" + tableName + "`)");
        try {
            SQLiteStatement stmt = sQLiteStatementPrepare;
    ...
    private static final TableInfo.Index readIndex(SQLiteConnection connection, String name, boolean unique) throws Exception {
        SQLiteStatement sQLiteStatementPrepare = connection.prepare("PRAGMA index_xinfo(`" + name + "`)");
        try {
            SQLiteStatement stmt = sQLiteStatementPrepare;
    ...
    public static final Set<String> parseFtsOptions(String createStatement) {
        ...
        while (i < $this$forEachIndexed$iv.length()) {
            ...
            switch (item$iv) {
                case '\"':
                case '\'':
                case ResolvedStyleKt.InheritedFlags /* 96 */:
                    z2 = z3;
        ...
        boolean z4 = z3;
        String strSubstring = argsString2.substring(lastDelimiterIndex2 + 1);
