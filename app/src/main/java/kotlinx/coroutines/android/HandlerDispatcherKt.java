...
public final class HandlerDispatcherKt {
    ...
    public static final Handler asHandler(Looper $this$asHandler, boolean async) throws IllegalAccessException, NoSuchMethodException, InvocationTargetException {
        if (!async) {
            return new Handler($this$asHandler);
        }
        if (Build.VERSION.SDK_INT >= 28) {
            Method factoryMethod = Handler.class.getDeclaredMethod("createAsync", Looper.class);
            Object objInvoke = factoryMethod.invoke(null, $this$asHandler);
    ...
    static {
        ...
        try {
            ...
            objM8929constructorimpl = Result.m8929constructorimpl(new HandlerContext(asHandler(Looper.getMainLooper(), true), null, 2, null));
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            objM8929constructorimpl = Result.m8929constructorimpl(ResultKt.createFailure(th));
        }
        Main = (HandlerDispatcher) (Result.m8935isFailureimpl(objM8929constructorimpl) ? null : objM8929constructorimpl);
    }
    ...
    public static final Object awaitFrameSlowPath(Continuation<? super Long> continuation) {
        ...
        if (Looper.myLooper() == Looper.getMainLooper()) {
            ...
            Dispatchers.getMain().mo10498dispatch(cont.get$context(), new Runnable() { // from class: kotlinx.coroutines.android.HandlerDispatcherKt$awaitFrameSlowPath$lambda$3$$inlined$Runnable$1
