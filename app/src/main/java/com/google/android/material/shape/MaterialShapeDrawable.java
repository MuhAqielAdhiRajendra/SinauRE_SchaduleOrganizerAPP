...
public class MaterialShapeDrawable extends Drawable implements TintAwareDrawable, Shapeable {
    ...
    protected void onBoundsChange(Rect bounds) {
        ...
        if (this.drawableState.shapeAppearance.isStateful() && !bounds.isEmpty()) {
    ...
    private void maybeDrawCompatShadow(Canvas canvas) {
        ...
        int pathExtraWidth = (int) (this.pathBounds.width() - drawableBounds.width());
        int pathExtraHeight = (int) (this.pathBounds.height() - drawableBounds.height());
        if (pathExtraWidth < 0 || pathExtraHeight < 0) {
            throw new IllegalStateException("Invalid shadow bounds. Check that the treatments result in a valid path. extra width: " + pathExtraWidth + " extra height: " + pathExtraHeight + " path bounds: " + this.pathBounds);
        }
        ...
        float shadowLeft = (drawableBounds.left - this.drawableState.shadowCompatRadius) - pathExtraWidth;
        float shadowTop = (drawableBounds.top - this.drawableState.shadowCompatRadius) - pathExtraHeight;
    ...
    protected void drawShape(Canvas canvas, Paint paint, Path path, RectF bounds) {
        drawShape(canvas, paint, path, this.drawableState.shapeAppearance.getDefaultShape(), this.springAnimatedCornerSizes, bounds);
    }

    private void drawShape(Canvas canvas, Paint paint, Path path, ShapeAppearanceModel shapeAppearanceModel, float[] cornerSizeOverrides, RectF bounds) {
    ...
    public float calculateRoundRectCornerSize() {
        float roundRectRadius = calculateRoundRectCornerSize(getBoundsAsRectF(), this.drawableState.shapeAppearance.getDefaultShape(), this.springAnimatedCornerSizes);
        if (roundRectRadius >= 0.0f) {
            return this.drawableState.interpolation * roundRectRadius;
        }
    ...
    protected final void calculatePathForSize(RectF bounds, Path path) {
        this.pathProvider.calculatePath(this.drawableState.shapeAppearance.getDefaultShape(), this.springAnimatedCornerSizes, this.drawableState.interpolation, bounds, this.pathShadowListener, path);
    }
    ...
    public void getOutline(Outline outline) {
        ...
        float roundRectRadius = calculateRoundRectCornerSize(bounds, this.drawableState.shapeAppearance.getDefaultShape(), this.springAnimatedCornerSizes);
        if (roundRectRadius >= 0.0f) {
    ...
    public boolean isStateful() {
        return super.isStateful() || (this.drawableState.tintList != null && this.drawableState.tintList.isStateful()) || ((this.drawableState.strokeTintList != null && this.drawableState.strokeTintList.isStateful()) || ((this.drawableState.strokeColor != null && this.drawableState.strokeColor.isStateful()) || ((this.drawableState.fillColor != null && this.drawableState.fillColor.isStateful()) || this.drawableState.shapeAppearance.isStateful())));
    }
    ...
    private void updateShape(int[] state, boolean skipAnimation) {
        ...
        ShapeAppearanceModel shapeAppearanceModel = this.drawableState.shapeAppearance.getShapeForState(state);
        if (MathUtils.areAllElementsEqual(this.springAnimatedCornerSizes) && shapeAppearanceModel.isRoundRect(getBoundsAsRectF())) {
            z = true;
        }
        this.isRoundRectCornerMorph = z;
        if (!this.isRoundRectCornerMorph) {
            this.pathDirty = true;
            this.strokePathDirty = true;
        }
    ...
    public boolean isRoundRect() {
        ShapeAppearanceModel shapeAppearanceModel = this.drawableState.shapeAppearance.getShapeForState(getState());
        return shapeAppearanceModel.isRoundRect(getBoundsAsRectF()) && (this.springAnimatedCornerSizes == null || this.isRoundRectCornerMorph);
    }
    ...
    public static class MaterialShapeDrawableState extends Drawable.ConstantState {
        ...
        public MaterialShapeDrawableState(MaterialShapeDrawableState orig) {
            ...
            this.useTintColorForShadow = false;
            this.paintStyle = Paint.Style.FILL_AND_STROKE;
            this.shapeAppearance = orig.shapeAppearance;
