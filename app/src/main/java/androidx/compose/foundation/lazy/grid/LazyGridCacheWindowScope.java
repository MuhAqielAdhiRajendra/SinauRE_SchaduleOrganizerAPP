...
final class LazyGridCacheWindowScope implements CacheWindowScope {
    ...
    static final Unit schedulePrefetch$lambda$0(Function2 $onItemPrefetched, int $lineIndex, LazyGridPrefetchResultScope $this$scheduleLinePrefetch) {
    ...
    public Object getVisibleLineKey(int indexInVisibleLines) {
        Object key;
        int laneIndex = getFirstVisibleLineIndex() + indexInVisibleLines;
        List<LazyGridItemInfo> visibleItemsInfo = getLayoutInfo().getVisibleItemsInfo();
        List target$iv = new ArrayList(visibleItemsInfo.size());
        int size = visibleItemsInfo.size();
        for (int index$iv$iv = 0; index$iv$iv < size; index$iv$iv++) {
        ...
        LazyGridItemInfo lazyGridItemInfo2 = (LazyGridItemInfo) CollectionsKt.firstOrNull($this$fastFilter$iv);
    ...
    public int getLastIndexInLine(int lineIndex) {
        ...
        if (measureResult.getPrefetchInfoRetriever().invoke(Integer.valueOf(lineIndex)).isEmpty()) {
            return -1;
        }
        return (((Number) ((Pair) CollectionsKt.first((List) r2)).getFirst()).intValue() + r2.size()) - 1;
    }
