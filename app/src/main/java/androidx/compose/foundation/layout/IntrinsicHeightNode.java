...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long mo1001calculateContentConstraintsl58MMJ0(MeasureScope $this$calculateContentConstraints_u2dl58MMJ0, Measurable measurable, long constraints) {
        ...
        if (this.height == IntrinsicSize.Min) {
            measuredHeight = measurable.minIntrinsicHeight(Constraints.m8103getMaxWidthimpl(constraints));
        } else {
            measuredHeight = measurable.maxIntrinsicHeight(Constraints.m8103getMaxWidthimpl(constraints));
        }
        if (measuredHeight < 0) {
            measuredHeight = 0;
        }
        return Constraints.INSTANCE.m8114fixedHeightOenEA2s(measuredHeight);
    }
