...
public final class TextUndoManagerKt {
    ...
    public static final void recordChanges(TextUndoManager $this$recordChanges, TextFieldCharSequence pre, TextFieldCharSequence post, TextFieldBuffer.ChangeList changes, boolean allowMerge) {
        ...
        if (changes.getChangeCount() == 1) {
            long preRange = changes.mo1718getOriginalRangejx7JFs(0);
            long postRange = changes.mo1719getRangejx7JFs(0);
            if (!TextRange.m7567getCollapsedimpl(preRange) || !TextRange.m7567getCollapsedimpl(postRange)) {
                $this$recordChanges.record(new TextUndoOperation(TextRange.m7571getMinimpl(preRange), TextRangeKt.m7580substringFDrldGo(pre, preRange), TextRangeKt.m7580substringFDrldGo(post, postRange), pre.getSelection(), post.getSelection(), 0L, allowMerge, 32, null));
            }
        }
    }
