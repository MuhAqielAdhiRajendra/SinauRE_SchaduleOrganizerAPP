...
public class ActivityWindowInfoCallbackController {
    ...
    static final void _init_$lambda$1(ActivityWindowInfoCallbackController this$0, androidx.window.extensions.embedding.EmbeddedActivityWindowInfo info) {
        Intrinsics.checkNotNullParameter(info, "info");
        ReentrantLock reentrantLock = this$0.globalLock;
        reentrantLock.lock();
        try {
            for (CallbackWrapper callbackWrapper : this$0.callbacks.values()) {
    ...
    public final void addCallback(Activity activity, Consumer<EmbeddedActivityWindowInfo> callback) {
        ...
        try {
            ...
            androidx.window.extensions.embedding.EmbeddedActivityWindowInfo $this$addCallback_u24lambda_u243_u24lambda_u242 = this.embeddingExtension.getEmbeddedActivityWindowInfo(activity);
            if ($this$addCallback_u24lambda_u243_u24lambda_u242 != null) {
                callbackWrapper.accept($this$addCallback_u24lambda_u243_u24lambda_u242);
            }
        } finally {
            reentrantLock.unlock();
        }
    ...
    public EmbeddedActivityWindowInfo translate$window_release(androidx.window.extensions.embedding.EmbeddedActivityWindowInfo info) {
        Intrinsics.checkNotNullParameter(info, "info");
        Rect parentHostBounds = new Rect(info.getTaskBounds());
        Rect boundsInParentHost = new Rect(info.getActivityStackBounds());
        boundsInParentHost.offset(-parentHostBounds.left, -parentHostBounds.top);
        return new EmbeddedActivityWindowInfo(info.isEmbedded(), parentHostBounds, boundsInParentHost);
    }
    ...
    public final class CallbackWrapper {
        ...
        public CallbackWrapper(ActivityWindowInfoCallbackController this$0, Activity activity, Consumer<EmbeddedActivityWindowInfo> callback) {
            Intrinsics.checkNotNullParameter(activity, "activity");
            Intrinsics.checkNotNullParameter(callback, "callback");
            this.this$0 = this$0;
        ...
        public final void accept(androidx.window.extensions.embedding.EmbeddedActivityWindowInfo extensionsActivityWindowInfo) {
            ...
            Activity updatedActivity = extensionsActivityWindowInfo.getActivity();
            Intrinsics.checkNotNullExpressionValue(updatedActivity, "getActivity(...)");
            if (!Intrinsics.areEqual(this.activity, updatedActivity)) {
                return;
            }
            EmbeddedActivityWindowInfo newInfo = this.this$0.translate$window_release(extensionsActivityWindowInfo);
            if (shouldReportInfo(newInfo)) {
                this.lastReportedInfo = newInfo;
                this.callback.accept(newInfo);
            }
        }

        private final boolean shouldReportInfo(EmbeddedActivityWindowInfo newInfo) {
            EmbeddedActivityWindowInfo it = this.lastReportedInfo;
            if (it != null) {
                if (it.getIsEmbedded() != newInfo.getIsEmbedded()) {
                    return true;
                }
                if (!newInfo.getIsEmbedded()) {
                    return false;
                }
                return !Intrinsics.areEqual(it, newInfo);
            }
