...
public class TransitionSet extends Transition {
    ...
    protected void runAnimators() {
        ...
        if (!this.mPlayTogether) {
            ...
            while (true) {
                ...
                Transition previousTransition = arrayList.get(i - 1);
            ...
            Transition firstTransition = arrayList.get(0);
    ...
    void setCurrentPlayTimeMillis(long playTimeMillis, long lastPlayTimeMillis) {
        ...
        if (this.mParent != null) {
            if (j2 < 0 && lastPlayTimeMillis < 0) {
                return;
            }
            if (j2 > duration && lastPlayTimeMillis > duration) {
                return;
            }
        }
        boolean isReverse = j2 < lastPlayTimeMillis;
        if ((j2 >= 0 && lastPlayTimeMillis < 0) || (j2 <= duration && lastPlayTimeMillis > duration)) {
            this.mEnded = false;
            notifyListeners(Transition.TransitionNotification.ON_START, isReverse);
        }
        if (this.mPlayTogether) {
            for (int i = 0; i < this.mTransitions.size(); i++) {
                this.mTransitions.get(i).setCurrentPlayTimeMillis(j2, lastPlayTimeMillis);
            }
            j = 0;
        } else {
            int startIndex = indexOfTransition(lastPlayTimeMillis);
            if (j2 >= lastPlayTimeMillis) {
                int i2 = startIndex;
                while (true) {
                ...
                int i3 = startIndex;
                while (i3 >= 0) {
                    Transition transition2 = this.mTransitions.get(i3);
        ...
        if (this.mParent != null) {
            if ((playTimeMillis > duration && lastPlayTimeMillis <= duration) || (playTimeMillis < j && lastPlayTimeMillis >= j)) {
    ...
    private Transition[] transitionsAsArray() {
        Transition[] transitions = this.mTransitionsCache;
        this.mTransitionsCache = null;
        if (transitions == null) {
            transitions = new Transition[this.mTransitions.size()];
        }
        return (Transition[]) this.mTransitions.toArray(transitions);
    }

    private void returnTransitionArrayToCache(Transition[] transitions) {
        Arrays.fill(transitions, (Object) null);
        this.mTransitionsCache = transitions;
    }
    ...
    public Transition mo8649clone() {
        TransitionSet clone = (TransitionSet) super.mo8649clone();
        clone.mTransitions = new ArrayList<>();
        int numTransitions = this.mTransitions.size();
        for (int i = 0; i < numTransitions; i++) {
            clone.addTransitionInternal(this.mTransitions.get(i).mo8649clone());
        }
        return clone;
    }
}
