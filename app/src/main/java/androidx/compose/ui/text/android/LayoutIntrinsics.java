...
public final class LayoutIntrinsics {
    ...
    private final CharSequence getCharSequenceForIntrinsicWidth() {
        if (this._charSequenceForIntrinsicWidth == null) {
            ...
            if (z) {
                CharSequence it = LayoutIntrinsics_androidKt.stripNonMetricAffectingCharacterStyleSpans(charSequence);
    ...
    private final float computeMinIntrinsicWidth() {
        ...
        PriorityQueue longestWordCandidates = new PriorityQueue(10, LayoutIntrinsics_androidKt.IntRangeComparator);
        int start = 0;
        for (int end = iterator.next(); end != -1; end = iterator.next()) {
            if (longestWordCandidates.size() < 10) {
                longestWordCandidates.add(new IntRange(start, end));
            } else {
                IntRange minPair = (IntRange) longestWordCandidates.peek();
                if (minPair != null && minPair.getLast() - minPair.getFirst() < end - start) {
        ...
        float desiredWidth = getDesiredWidth(range.getFirst(), range.getLast());
        while (it.hasNext()) {
            IntRange range2 = (IntRange) it.next();
            desiredWidth = Math.max(desiredWidth, getDesiredWidth(range2.getFirst(), range2.getLast()));
        }
        return desiredWidth;
    }
    ...
    private final float computeMaxIntrinsicWidth() {
        ...
        if (LayoutIntrinsics_androidKt.shouldIncreaseMaxIntrinsic(desiredWidth, this.charSequence, this.textPaint)) {
