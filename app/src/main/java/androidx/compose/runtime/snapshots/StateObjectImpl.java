...
public abstract class StateObjectImpl implements StateObject {
    ...
    public final void m4724recordReadInh_f27i8$runtime(int reader) {
        int old;
        int reader$iv;
        do {
            old = ReaderKind.m4712constructorimpl(this.readerKind.get());
            if ((old & reader) != 0) {
                return;
            } else {
                reader$iv = ReaderKind.m4712constructorimpl(old | reader);
            }
        } while (!this.readerKind.compareAndSet(old, reader$iv));
    }

    /* JADX INFO: renamed from: isReadIn-h_f27i8$runtime, reason: not valid java name */
    public final boolean m4723isReadInh_f27i8$runtime(int reader) {
        int arg0$iv = ReaderKind.m4712constructorimpl(this.readerKind.get());
