...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void setView(float position, MotionWidget view, int[] toUse, double[] data, double[] slope, double[] cycle) {
        ...
        if (this.mRelativeToController != null) {
            ...
            float drx = vel[0];
            float dry = vel[c];
            float pos_x = (float) ((((double) rx) + (Math.sin(v_y4) * ((double) radius))) - ((double) (v_width / 2.0f)));
            float pos_y = (float) ((((double) ry) - (Math.cos(v_y4) * ((double) radius))) - ((double) (v_height / 2.0f)));
            float dpos_x = (float) (((double) drx) + (Math.sin(v_y4) * ((double) dradius)) + (((double) dangle) * Math.cos(v_y4) * ((double) radius)));
            float dpos_y = (float) ((((double) dry) - (((double) dradius) * Math.cos(v_y4))) + (Math.sin(v_y4) * ((double) radius) * ((double) dangle)));
            v_x2 = pos_x;
            if (slope.length >= 2) {
                slope[0] = dpos_x;
                slope[c] = dpos_y;
            ...
            if (!Float.isNaN(path_rotate)) {
                ...
                float rot2 = 0.0f + ((float) (((double) path_rotate) + Math.toDegrees(Math.atan2(dy, dx))));
