...
class FragmentStateManager {
    ...
    FragmentStateManager(FragmentLifecycleCallbacksDispatcher dispatcher, FragmentStore fragmentStore, Fragment retainedFragment, FragmentState fs) {
        ...
        if (bundle != null) {
            fragment.mSavedFragmentState = fs.mSavedFragmentState;
        } else {
    ...
    int computeExpectedState() {
        ...
        if (this.mFragment.mFromLayout) {
            ...
            if (z) {
                maxState = Math.max(i, 2);
                if (this.mFragment.mView != null && this.mFragment.mView.getParent() == null) {
                    maxState = Math.min(maxState, 2);
                }
            } else {
                maxState = i < 4 ? Math.min(maxState, this.mFragment.mState) : Math.min(maxState, 1);
            }
        }
        ...
        try {
            ...
            while (true) {
                ...
                if (newState != this.mFragment.mState) {
                    ...
                    Fragment fragment = this.mFragment;
                    if (newState > i) {
                        int nextStep = fragment.mState + 1;
                        switch (nextStep) {
                            case 0:
                                attach();
                                break;
                        ...
                        int nextStep2 = fragment.mState - 1;
                        switch (nextStep2) {
                            case -1:
                                detach();
                                break;
    ...
    void restoreState(ClassLoader classLoader) {
        ...
        Fragment fragment = this.mFragment;
        if (bool == null) {
            fragment.mUserVisibleHint = this.mFragment.mSavedFragmentState.getBoolean(USER_VISIBLE_HINT_TAG, true);
        } else {
            fragment.mUserVisibleHint = this.mFragment.mSavedUserVisibleHint.booleanValue();
    ...
    void createView() {
        ...
        if (this.mFragment.mView != null) {
            ...
            if (zIsAttachedToWindow) {
                ViewCompat.requestApplyInsets(fragment2.mView);
            } else {
                final View fragmentView = fragment2.mView;
    ...
    void destroy() {
        ...
        Fragment fragment = this.mFragment;
        if (shouldDestroy) {
            FragmentHostCallback<?> host = fragment.mHost;
        ...
        if (fragment.mTargetWho != null && (target = this.mFragmentStore.findActiveFragment(this.mFragment.mTargetWho)) != null && target.mRetainInstance) {
