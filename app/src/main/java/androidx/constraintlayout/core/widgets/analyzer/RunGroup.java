...
class RunGroup {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (dependency instanceof DependencyNode) {
                ...
                if (nextNode.mRun != run) {
                    position = Math.max(position, traverseStart(nextNode, ((long) nextNode.mMargin) + startPosition));
                }
            }
        }
        if (node == run.start) {
            long dimension = run.getWrapDimension();
            return Math.max(Math.max(position, traverseStart(run.end, startPosition + dimension)), (startPosition + dimension) - ((long) run.end.mMargin));
        }
        return position;
    }
    ...
    private long traverseEnd(DependencyNode node, long startPosition) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (dependency instanceof DependencyNode) {
                DependencyNode nextNode = (DependencyNode) dependency;
                if (nextNode.mRun != run) {
                    position = Math.min(position, traverseEnd(nextNode, ((long) nextNode.mMargin) + startPosition));
                }
            }
        }
        if (node == run.end) {
            long dimension = run.getWrapDimension();
            return Math.min(Math.min(position, traverseEnd(run.start, startPosition - dimension)), (startPosition - dimension) - ((long) run.start.mMargin));
        }
        return position;
    }
    ...
    public long computeWrapSize(ConstraintWidgetContainer container, int orientation) {
        ...
        if (runWithStartTarget && runWithEndTarget) {
            ...
            return (((long) this.mFirstRun.start.mMargin) + runDimension) - ((long) this.mFirstRun.end.mMargin);
        }
        if (runWithStartTarget) {
            long maxPosition2 = traverseStart(this.mFirstRun.start, this.mFirstRun.start.mMargin);
            long runDimension2 = ((long) this.mFirstRun.start.mMargin) + dimension;
        ...
        if (runWithEndTarget) {
            ...
            long runDimension3 = ((long) (-this.mFirstRun.end.mMargin)) + dimension;
            return Math.max(-minPosition2, runDimension3);
        }
        return (((long) widgetRun2.start.mMargin) + this.mFirstRun.getWrapDimension()) - ((long) this.mFirstRun.end.mMargin);
    }
            ...
            if (dependency2 instanceof DependencyNode) {
                ...
                if (node2.mRun != run && node2 == node2.mRun.start) {
                    if (run instanceof ChainRun) {
                        ...
                        for (WidgetRun widgetChainRun2 : chainRun2.mWidgets) {
