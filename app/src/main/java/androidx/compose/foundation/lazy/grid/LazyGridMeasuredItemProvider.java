...
public abstract class LazyGridMeasuredItemProvider extends LazyLayoutMeasuredItemProvider<LazyGridMeasuredItem> {
    ...
    public abstract LazyGridMeasuredItem mo1206createItemO3s9Psw(int index, Object key, Object contentType, int crossAxisSize, int mainAxisSpacing, List<? extends Placeable> placeables, long constraints, int lane, int span);
    ...
    public LazyGridMeasuredItem mo1184getAndMeasurehBUhpc(int index, int lane, int span, long constraints) {
        return m1212getAndMeasurem8Kt_7k(index, constraints, lane, span, this.defaultMainAxisSpacing);
    }

    /* JADX INFO: renamed from: getAndMeasure-m8Kt_7k, reason: not valid java name */
    public final LazyGridMeasuredItem m1212getAndMeasurem8Kt_7k(int index, long constraints, int lane, int span, int mainAxisSpacing) {
        int crossAxisSize;
        Object key = this.itemProvider.getKey(index);
        Object contentType = this.itemProvider.getContentType(index);
        List<Placeable> list = m1246getPlaceables3p2s80s(this.measureScope, index, constraints);
        if (Constraints.m8101getHasFixedWidthimpl(constraints)) {
            crossAxisSize = Constraints.m8105getMinWidthimpl(constraints);
        } else {
            boolean value$iv = Constraints.m8100getHasFixedHeightimpl(constraints);
            if (!value$iv) {
                InlineClassHelperKt.throwIllegalArgumentException("does not have fixed height");
            }
            crossAxisSize = Constraints.m8104getMinHeightimpl(constraints);
        }
        return mo1206createItemO3s9Psw(index, key, contentType, crossAxisSize, mainAxisSpacing, list, constraints, lane, span);
    }
