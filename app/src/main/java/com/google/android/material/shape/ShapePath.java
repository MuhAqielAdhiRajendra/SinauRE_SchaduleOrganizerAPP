    ...
    static class InnerCornerShadowOperation extends ShadowCompatOperation {
        ...
        public void draw(Matrix transform, ShadowRenderer shadowRenderer, int shadowElevation, Canvas canvas) {
            ...
            if (sweepAngle <= 0.0f) {
                ...
                double length2 = Math.hypot(this.operation2.x - this.operation1.x, this.operation2.y - this.operation1.y);
                float arcRadius = (float) Math.min(shadowElevation, Math.min(length1, length2));
                ...
                if (length1 <= retractLength) {
                    ...
                    i = shadowElevation;
                    shadowRenderer.drawEdgeShadow(canvas, this.renderMatrix, rect1, i);
                }
                ...
                shadowRenderer.drawInnerCornerShadow(canvas, this.renderMatrix, rect, (int) arcRadius, 450.0f, sweepAngle, new float[]{(float) (((double) arcRadius) + retractLength), arcRadius * 2.0f});
                if (length2 > retractLength) {
                    ...
                    shadowRenderer.drawEdgeShadow(canvas, this.renderMatrix, rect2, i);
                }
            }
        }
