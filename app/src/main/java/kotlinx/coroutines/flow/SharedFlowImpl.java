...
public class SharedFlowImpl<T> extends AbstractSharedFlow<SharedFlowSlot> implements MutableSharedFlow<T>, CancellableFlow<T>, FusibleFlow<T> {
    ...
    /* JADX WARN: Type inference failed for: r3v12, types: [kotlinx.coroutines.flow.FlowCollector] */
    /* JADX WARN: Type inference failed for: r3v13, types: [java.lang.Object, kotlinx.coroutines.flow.FlowCollector] */
    ...
    /* JADX WARN: Type inference failed for: r9v9, types: [java.lang.Object, kotlinx.coroutines.flow.SharedFlowSlot] */
    /* JADX WARN: Unsupported multi-entry loop pattern (BACK_EDGE: B:41:0x00d4 -> B:29:0x009d). Please report as a decompilation issue!!! */
    /*
    ...
    */
    ...
    public final Object emitSuspend(T t, Continuation<? super Unit> continuation) {
        ...
        synchronized (this) {
            if (!tryEmitLocked(t)) {
                Emitter it = new Emitter(this, getHead() + ((long) getTotalSize()), t, cont);
                enqueueLocked(it);
                this.queueSize++;
        ...
        for (Continuation<Unit> continuation2 : continuationArrFindSlotsToResumeLocked) {
            if (continuation2 != null) {
                ...
                continuation2.resumeWith(Result.m8929constructorimpl(Unit.INSTANCE));
            }
        }
    ...
    */
    ...
    public final Continuation<Unit>[] findSlotsToResumeLocked(Continuation<Unit>[] resumesIn) {
        ...
        Object[] objArr = resumesIn;
        int resumeCount2 = resumesIn.length;
        ...
        if (((AbstractSharedFlow) this_$iv).nCollectors != 0 && ($this$forEach$iv$iv = ((AbstractSharedFlow) this_$iv).slots) != null) {
            ...
            while (i < length) {
                ...
                if (element$iv$iv != null) {
                    SharedFlowSlot slot = (SharedFlowSlot) element$iv$iv;
                    Continuation<? super Unit> continuation = slot.cont;
                    if (continuation == null || sharedFlowImpl.tryPeekLocked(slot) < 0) {
                        int resumeCount3 = resumeCount2;
                        resumeCount2 = resumeCount3;
                    } else {
                        if (resumeCount2 >= objArr.length) {
                            resumeCount = resumeCount2;
                            int resumeCount4 = objArr.length * 2;
                            Object[] objArrCopyOf = Arrays.copyOf(objArr, Math.max(2, resumeCount4));
                        ...
                        ((Continuation[]) objArr)[resumeCount] = continuation;
                        slot.cont = null;
                    }
                }
                i++;
