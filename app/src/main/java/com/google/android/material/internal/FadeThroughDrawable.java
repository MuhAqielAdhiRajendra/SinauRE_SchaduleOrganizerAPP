...
public class FadeThroughDrawable extends Drawable {
    ...
    public FadeThroughDrawable(Drawable fadeOutDrawable, Drawable fadeInDrawable) {
        ...
        if (fadeOutDrawable != null) {
            emptyDrawable = fadeOutDrawable.getConstantState().newDrawable().mutate();
        } else {
            emptyDrawable = new EmptyDrawable();
        }
        this.fadeOutDrawable = emptyDrawable;
        if (fadeInDrawable != null) {
            emptyDrawable2 = fadeInDrawable.getConstantState().newDrawable().mutate();
        } else {
            emptyDrawable2 = new EmptyDrawable();
        }
        this.fadeInDrawable = emptyDrawable2;
        if (fadeOutDrawable != null) {
            outLayoutDir = DrawableCompat.getLayoutDirection(fadeOutDrawable);
        } else {
            outLayoutDir = 3;
        }
        int inLayoutDir = fadeInDrawable != null ? DrawableCompat.getLayoutDirection(fadeInDrawable) : 3;
