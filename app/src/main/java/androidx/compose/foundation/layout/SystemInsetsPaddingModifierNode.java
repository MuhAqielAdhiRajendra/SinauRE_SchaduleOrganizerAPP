...
final class SystemInsetsPaddingModifierNode extends InsetsPaddingModifierNode implements LayoutModifierNode {
    ...
    public void onAttach() {
        ...
        WindowInsetsHolder holder = WindowInsetsHolder.INSTANCE.getOrCreateFor(view);
        holder.incrementAccessors(view);
        update(this.insetsGetter.invoke(holder));
        this.windowInsetsHolder = holder;
        super.onAttach();
    }
    ...
    public final void update(Function1<? super WindowInsetsHolder, ? extends WindowInsets> insetsGetter) {
        if (this.insetsGetter != insetsGetter) {
            ...
            WindowInsetsHolder holder = this.windowInsetsHolder;
            if (holder != null) {
                update(this.insetsGetter.invoke(holder));
            }
        }
