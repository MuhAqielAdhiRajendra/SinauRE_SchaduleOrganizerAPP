...
final class SplineBasedFloatDecayAnimationSpec implements FloatDecayAnimationSpec {
    ...
    public final float flingDistance(float velocity) {
        ...
        return ((float) (((double) (WindowInsetsConnection_androidKt.PlatformFlingScrollFriction * this.magicPhysicalCoefficient)) * Math.exp((WindowInsetsConnection_androidKt.DecelerationRate / WindowInsetsConnection_androidKt.DecelMinusOne) * l))) * Math.signum(velocity);
    }
    ...
    public float getValueFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        ...
        return (AndroidFlingSpline.FlingResult.m735getDistanceCoefficientimpl(AndroidFlingSpline.INSTANCE.m730flingPositionLfoxSSI(splinePos)) * distance) + initialValue;
    }

    @Override // androidx.compose.animation.core.FloatDecayAnimationSpec
    public long getDurationNanos(float initialValue, float initialVelocity) {
        double l = getSplineDeceleration(initialVelocity);
        return (long) (Math.exp(l / WindowInsetsConnection_androidKt.DecelMinusOne) * 1.0E9d);
    }
    ...
    public float getVelocityFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        ...
        float splinePos = duration > 0 ? playTimeNanos / duration : 1.0f;
        float distance = flingDistance(initialVelocity);
        return ((AndroidFlingSpline.FlingResult.m736getVelocityCoefficientimpl(AndroidFlingSpline.INSTANCE.m730flingPositionLfoxSSI(splinePos)) * distance) / duration) * 1.0E9f;
    }
}
