...
public class SortedList<T> {
    ...
    private int add(T item, boolean notify) {
        ...
        if (index == -1) {
            ...
            if (this.mCallback.areItemsTheSame(existing, item)) {
                boolean zAreContentsTheSame = this.mCallback.areContentsTheSame(existing, item);
                T[] tArr = this.mData;
    ...
    public int indexOf(T item) {
        ...
        T[] tArr2 = this.mData;
        if (tArr != null) {
            int index = findIndexOf(item, tArr2, 0, this.mNewDataStart, 4);
            if (index != -1) {
                return index;
            }
        ...
        return findIndexOf(item, tArr2, 0, this.mSize, 4);
    }
