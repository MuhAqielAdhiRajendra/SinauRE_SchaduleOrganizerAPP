...
class FragmentStore {
    ...
    void makeActive(FragmentStateManager newlyActive) {
        ...
        if (f.mRetainInstanceChangedWhileDetached) {
            boolean z = f.mRetainInstance;
            FragmentManagerViewModel fragmentManagerViewModel = this.mNonConfig;
            if (z) {
                fragmentManagerViewModel.addRetainedFragment(f);
            } else {
                fragmentManagerViewModel.removeRetainedFragment(f);
            }
    ...
    FragmentState getSavedState(String who) {
        return this.mSavedState.get(who);
    }

    FragmentState setSavedState(String who, FragmentState fragmentState) {
        HashMap<String, FragmentState> map = this.mSavedState;
        if (fragmentState != null) {
            return map.put(who, fragmentState);
        }
        return map.remove(who);
    }

    void restoreSaveState(ArrayList<FragmentState> savedState) {
