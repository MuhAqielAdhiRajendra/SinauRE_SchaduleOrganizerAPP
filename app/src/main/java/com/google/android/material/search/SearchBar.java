...
public class SearchBar extends Toolbar {
    ...
    public SearchBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        this.liftColorListener = new AppBarLayout.LiftOnScrollProgressListener() { // from class: com.google.android.material.search.SearchBar.1
            ...
            public void onUpdate(float elevation, int appBarLayoutColor, float progress) {
                if (SearchBar.this.liftOnScrollColor != null) {
                    int mixedColor = MaterialColors.layer(SearchBar.this.backgroundColor, SearchBar.this.liftOnScrollColor.getDefaultColor(), progress);
                    SearchBar.this.backgroundShape.setFillColor(ColorStateList.valueOf(mixedColor));
                }
            }
        };
        ...
        this.backgroundColor = a.getColor(R.styleable.SearchBar_backgroundTint, 0);
        this.liftOnScrollColor = MaterialResources.getColorStateList(context2, a, R.styleable.SearchBar_liftOnScrollColor);
        ...
        this.textCentered = a.getBoolean(R.styleable.SearchBar_textCentered, false);
        this.liftOnScroll = a.getBoolean(R.styleable.SearchBar_liftOnScroll, false);
        this.maxWidth = a.getDimensionPixelSize(R.styleable.SearchBar_android_maxWidth, -1);
        this.adaptiveMaxWidthEnabled = a.getBoolean(R.styleable.SearchBar_adaptiveMaxWidthEnabled, false);
        this.startSiblingViewId = a.getResourceId(R.styleable.SearchBar_startSiblingViewId, -1);
        this.endSiblingViewId = a.getResourceId(R.styleable.SearchBar_endSiblingViewId, -1);
        a.recycle();
        ...
        initBackground(shapeAppearanceModel, this.backgroundColor, elevation, strokeWidth, strokeColor);
    }
    ...
    private void initBackground(ShapeAppearanceModel shapeAppearance, int backgroundColor, float elevation, float strokeWidth, int strokeColor) {
        ...
        RippleDrawable rippleDrawable = new RippleDrawable(ColorStateList.valueOf(rippleColor), this.backgroundShape, this.backgroundShape);
    ...
    private Drawable maybeTintNavigationIcon(Drawable navigationIcon) {
        ...
        wrappedNavigationIcon.setTint(navigationIconColorAttr2);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.maxWidth >= 0 && availableWidth > this.maxWidth) {
            widthMeasureSpec = View.MeasureSpec.makeMeasureSpec(this.maxWidth, measureMode);
        } else if (this.adaptiveMaxWidthEnabled && availableWidth > this.adaptiveMaxWidthParentBreakpoint) {
            int newWidth = Math.max(this.adaptiveMaxWidthParentBreakpoint, Math.round(availableWidth * 0.5f));
            widthMeasureSpec = View.MeasureSpec.makeMeasureSpec(newWidth, measureMode);
        }
    ...
    private void layoutTextViewCenterAvoidToolbarViewsAndPadding() {
        ...
        int textViewRight = this.textView.getMeasuredWidth() + textViewLeft;
        int left = textViewLeft + textViewContainerLeft2;
        int right = textViewContainerLeft2 + textViewRight;
        ...
        int measuredWidth = right3 - (getMeasuredWidth() - getPaddingRight());
        int textViewContainerRight2 = right3 - (getMeasuredWidth() - getContentInsetRight());
        int paddingRightShift = Math.max(measuredWidth, textViewContainerRight2);
        int totalShift = ((leftShift - rightShift) + Math.max(paddingLeftShift, 0)) - Math.max(paddingRightShift, 0);
        int paddingRightShift2 = textViewContainerRight + totalShift;
        this.textViewContainer.layout(textViewContainerLeft + totalShift, textViewContainerTop, paddingRightShift2, textViewContainerBottom);
    }
    ...
    public void setStartSiblingViewId(int startSiblingViewId) {
    ...
    public void setEndSiblingViewId(int endSiblingViewId) {
