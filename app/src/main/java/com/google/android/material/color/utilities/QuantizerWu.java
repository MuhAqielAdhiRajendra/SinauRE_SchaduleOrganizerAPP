...
public final class QuantizerWu implements Quantizer {
    ...
    Boolean cut(Box one, Box two) {
        ...
        one.vol = (one.b1 - one.b0) * (one.g1 - one.g0) * (one.r1 - one.r0);
        two.vol = (two.b1 - two.b0) * (two.g1 - two.g0) * (two.r1 - two.r0);
    ...
    MaximizeResult maximize(Box cube, Direction direction, int first, int last, int wholeR, int wholeG, int wholeB, int wholeW) {
        ...
        int bottomG = bottom(box, direction, quantizerWu.momentsG);
        int bottomB = bottom(box, direction, quantizerWu.momentsB);
        ...
        while (i < last) {
            ...
            int halfR = bottomR2 + top(box, direction, i, quantizerWu.momentsR);
            int halfG = top(box, direction, i, quantizerWu.momentsG) + bottomG;
            int halfB = top(box, direction, i, quantizerWu.momentsB) + bottomB;
            int halfW = top(box, direction, i, quantizerWu.weights) + bottomW;
            if (halfW != 0) {
                double tempNumerator = (halfR * halfR) + (halfG * halfG) + (halfB * halfB);
                double tempNumerator2 = halfW;
                double temp = tempNumerator / tempNumerator2;
                halfR = wholeR - halfR;
                halfG = wholeG - halfG;
                halfB = wholeB - halfB;
                halfW = wholeW - halfW;
                if (halfW != 0) {
                    double tempDenominator = (halfR * halfR) + (halfG * halfG) + (halfB * halfB);
                    double tempNumerator3 = halfW;
                    double temp2 = temp + (tempDenominator / tempNumerator3);
                    if (temp2 > max) {
                        max = temp2;
                        cut = i;
                    }
