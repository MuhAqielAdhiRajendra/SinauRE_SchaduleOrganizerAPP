...
public final class TransformedTextFieldStateKt {
    public static final <R> R getIndexTransformationType(TransformedTextFieldState $this$getIndexTransformationType, int transformedQueryIndex, Function3<? super IndexTransformationType, ? super TextRange, ? super TextRange, ? extends R> function3) {
        ...
        long untransformed = $this$getIndexTransformationType.m1892mapFromTransformedjx7JFs(transformedQueryIndex);
        long retransformed = $this$getIndexTransformationType.m1895mapToTransformedGEjPoXI(untransformed);
        if (TextRange.m7567getCollapsedimpl(untransformed) && TextRange.m7567getCollapsedimpl(retransformed)) {
            type = IndexTransformationType.Untransformed;
        } else if (!TextRange.m7567getCollapsedimpl(untransformed) && !TextRange.m7567getCollapsedimpl(retransformed)) {
        ...
        return function3.invoke(type, TextRange.m7561boximpl(untransformed), TextRange.m7561boximpl(retransformed));
    }
}
