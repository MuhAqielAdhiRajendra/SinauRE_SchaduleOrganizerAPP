...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString text, long selection) {
        ...
        this.selectionStart = TextRange.m7571getMinimpl(selection);
        this.selectionEnd = TextRange.m7570getMaximpl(selection);
    ...
    public final TextRange m7735getCompositionMzsxiRA$ui_text() {
        if (hasComposition$ui_text()) {
            return TextRange.m7561boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* JADX INFO: renamed from: getSelection-d9O1mEE$ui_text, reason: not valid java name */
    public final long m7736getSelectiond9O1mEE$ui_text() {
    ...
    public final void delete$ui_text(int start, int end) {
        ...
        long newSelection = EditingBufferKt.m7737updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), deleteRange);
        setSelectionStart(TextRange.m7571getMinimpl(newSelection));
        setSelectionEnd(TextRange.m7570getMaximpl(newSelection));
        if (hasComposition$ui_text()) {
            long compositionRange = TextRangeKt.TextRange(this.compositionStart, this.compositionEnd);
            long newComposition = EditingBufferKt.m7737updateRangeAfterDeletepWDy79M(compositionRange, deleteRange);
            if (TextRange.m7567getCollapsedimpl(newComposition)) {
                commitComposition$ui_text();
            } else {
                this.compositionStart = TextRange.m7571getMinimpl(newComposition);
                this.compositionEnd = TextRange.m7570getMaximpl(newComposition);
            }
        }
    }
