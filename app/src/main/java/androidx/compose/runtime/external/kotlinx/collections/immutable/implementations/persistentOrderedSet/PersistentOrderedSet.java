...
public final class PersistentOrderedSet<E> extends AbstractSet<E> implements PersistentSet<E> {
    ...
    public PersistentSet<E> remove(E element) {
        ...
        PersistentHashMap newMap = this.hashMap.remove(element);
        if (links.getHasPrevious()) {
            V v = newMap.get(links.getPrevious());
            Intrinsics.checkNotNull(v);
            Links previousLinks = (Links) v;
            newMap = newMap.put(links.getPrevious(), previousLinks.withNext(links.getNext()));
        }
        if (links.getHasNext()) {
            V v2 = newMap.get(links.getNext());
            Intrinsics.checkNotNull(v2);
            Links nextLinks = (Links) v2;
            newMap = newMap.put(links.getNext(), nextLinks.withPrevious(links.getPrevious()));
        }
