...
public final class BlockDropShadowNode extends Modifier.Node implements DrawModifierNode, ObserverModifierNode, DropShadowScope {
    ...
    public void draw(ContentDrawScope $this$draw) throws Throwable {
        DropShadowPainter $this$draw_u24lambda_u240 = obtainPainter();
    ...
    private final DropShadowPainter obtainPainter() {
        ...
        DropShadowPainter painter = this.shadowPainter;
        ...
        DropShadowPainter it2 = DelegatableNodeKt.requireGraphicsContext(this).getShadowContext().createDropShadowPainter(this.shape, it);
    ...
    public boolean equals(Object other) {
        ...
        if (getAlpha() == ((BlockDropShadowNode) other).getAlpha()) {
            z = true;
        } else {
            z = false;
        }
        if (!z || !Intrinsics.areEqual(this.shape, ((BlockDropShadowNode) other).shape) || this.block != ((BlockDropShadowNode) other).block) {
            return false;
        }
        if (getRadius() == ((BlockDropShadowNode) other).getRadius()) {
            z2 = true;
        } else {
            z2 = false;
        }
        ...
        if (getSpread() == ((BlockDropShadowNode) other).getSpread()) {
            z3 = true;
        } else {
            z3 = false;
        }
        if (z3 && Offset.m5065equalsimpl0(getOffset(), ((BlockDropShadowNode) other).getOffset()) && Color.m5314equalsimpl0(getColor(), ((BlockDropShadowNode) other).getColor()) && Intrinsics.areEqual(getBrush(), ((BlockDropShadowNode) other).getBrush()) && BlendMode.m5222equalsimpl0(getBlendMode(), ((BlockDropShadowNode) other).getBlendMode())) {
            return true;
        }
        return false;
    }
