...
public final class SavedStateHandle {
    ...
    private final <T> MutableLiveData<T> getLiveDataInternal(String key, boolean hasInitialValue, T initialValue) {
        ...
        if (savingStateLiveData2 == null) {
            ...
            map.put(key, savingStateLiveData);
        } else {
    ...
    public final <T> StateFlow<T> getStateFlow(String key, T initialValue) {
        ...
        if (zContainsKey) {
            return FlowKt.asStateFlow(savedStateHandleImpl.getMutableStateFlow(key, initialValue));
        }
        return savedStateHandleImpl.getStateFlow(key, initialValue);
    }
    ...
    public final <T> void set(String key, T value) {
        ...
        if (!INSTANCE.validateValue(value)) {
            StringBuilder sbAppend = new StringBuilder().append("Can't put value with type ");
            Intrinsics.checkNotNull(value);
            throw new IllegalArgumentException(sbAppend.append(value.getClass()).append(" into saved state").toString().toString());
        }
    ...
    public static final class Companion {
        ...
        public final SavedStateHandle createHandle(Bundle restoredState, Bundle defaultState) {
            Bundle initialState = restoredState == null ? defaultState : restoredState;
            ...
            Bundle $this$createHandle_u24lambda_u240 = SavedStateReader.m8522constructorimpl($this$read$iv);
            return new SavedStateHandle(SavedStateReader.m8603toMapimpl($this$createHandle_u24lambda_u240));
        }
    ...
    @JvmStatic
    public static final SavedStateHandle createHandle(Bundle restoredState, Bundle defaultState) {
        return INSTANCE.createHandle(restoredState, defaultState);
    }
}
