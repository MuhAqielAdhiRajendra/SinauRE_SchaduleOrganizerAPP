...
final class ArrayDecoders {
    ...
    static int decodeVarint64(long firstByte, byte[] data, int position, Registers registers) {
        ...
        int shift = 7;
        long value2 = value | (((long) (next & ByteCompanionObject.MAX_VALUE)) << 7);
        while (next < 0) {
            next = data[position2];
            shift += 7;
            value2 |= ((long) (next & ByteCompanionObject.MAX_VALUE)) << shift;
            position2++;
        }
    ...
    static int decodeFixed32(byte[] data, int position) {
        return (data[position] & UByte.MAX_VALUE) | ((data[position + 1] & UByte.MAX_VALUE) << 8) | ((data[position + 2] & UByte.MAX_VALUE) << 16) | ((data[position + 3] & UByte.MAX_VALUE) << 24);
    }
    ...
    static int decodeMessageField(Schema schema, byte[] bArr, int position, int limit, Registers registers) throws IOException {
        int position2;
        ...
        schema.mergeFrom(result, bArr, position2, position2 + i, registers);
    ...
    static int decodeExtension(int tag, byte[] data, int position, int limit, GeneratedMessageLite.ExtendableMessage<?, ?> message, GeneratedMessageLite.GeneratedExtension<?, ?> extension, UnknownFieldSchema<UnknownFieldSetLite, UnknownFieldSetLite> unknownFieldSchema, Registers registers) throws IOException {
        ...
        if (extension.descriptor.isRepeated() && extension.descriptor.isPacked()) {
            switch (extension.getLiteType()) {
                ...
                case WireFormat.FieldType.ENUM:
                    ...
                    UnknownFieldSetLite unknownFields2 = (UnknownFieldSetLite) SchemaUtil.filterUnknownEnumList(fieldNumber, (List<Integer>) list10, extension.descriptor.getEnumType(), unknownFields, (UnknownFieldSchema<UT, UnknownFieldSetLite>) unknownFieldSchema);
        ...
        if (extension.getLiteType() == WireFormat.FieldType.ENUM) {
            ...
            if (enumValue == null) {
                UnknownFieldSetLite unknownFields3 = message.unknownFields;
                if (unknownFields3 == UnknownFieldSetLite.getDefaultInstance()) {
                    unknownFields3 = UnknownFieldSetLite.newInstance();
                    message.unknownFields = unknownFields3;
                }
                SchemaUtil.storeUnknownEnum(fieldNumber, registers.int1, unknownFields3, unknownFieldSchema);
                return position2;
            }
            ...
            switch (extension.getLiteType()) {
                ...
                case WireFormat.FieldType.GROUP:
                    ...
                    position2 = decodeGroupField(Protobuf.getInstance().schemaFor((Class) extension.getMessageDefaultInstance().getClass()), data, position, limit, endTag, registers);
                    value = registers.object1;
                    break;
                case WireFormat.FieldType.MESSAGE:
                    position2 = decodeMessageField(Protobuf.getInstance().schemaFor((Class) extension.getMessageDefaultInstance().getClass()), data, position, limit, registers);
                    value = registers.object1;
                    break;
                default:
                    position2 = position;
    ...
    static int decodeUnknownField(int tag, byte[] data, int position, int limit, UnknownFieldSetLite unknownFields, Registers registers) throws InvalidProtocolBufferException {
        ...
        switch (WireFormat.getTagWireType(tag)) {
            ...
            case 2:
                ...
                int position4 = registers.int1;
                if (position4 < 0) {
                    throw InvalidProtocolBufferException.negativeSize();
                }
                if (position4 > data.length - position3) {
                    throw InvalidProtocolBufferException.truncatedMessage();
                }
                if (position4 == 0) {
                    unknownFields.storeField(tag, ByteString.EMPTY);
                } else {
                    unknownFields.storeField(tag, ByteString.copyFrom(data, position3, position4));
                }
