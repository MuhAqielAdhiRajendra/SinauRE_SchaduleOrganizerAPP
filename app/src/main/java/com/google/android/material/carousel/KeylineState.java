...
public final class KeylineState {
    ...
    private KeylineState(float itemSize, List<Keyline> keylines, int firstFocalKeylineIndex, int lastFocalKeylineIndex, int carouselSize) {
    ...
    static KeylineState lerp(KeylineState from, KeylineState to, float progress) {
        ...
        return new KeylineState(from.getItemSize(), keylines, focalKeylineFirstIndex, focalKeylineLastIndex, from.carouselSize);
    }

    static KeylineState reverse(KeylineState keylineState, int carouselSize) {
        Builder builder = new Builder(keylineState.getItemSize(), carouselSize);
        float start = (carouselSize - keylineState.getLastKeyline().locOffset) - (keylineState.getLastKeyline().maskedItemSize / 2.0f);
    ...
    public static final class Builder {
        ...
        public Builder addKeyline(float offsetLoc, float mask, float maskedItemSize, boolean isFocal, boolean isAnchor, float cutoff, float leftOrTopPaddingShift, float rightOrBottomPaddingShift) {
            ...
            Keyline keyline = this.tmpFirstFocalKeyline;
            if (isFocal) {
                if (keyline == null) {
                ...
                if (keyline == null && tmpKeyline.maskedItemSize < this.lastKeylineMaskedSize) {
        ...
        public Builder addKeyline(float offsetLoc, float mask, float maskedItemSize, boolean isFocal, boolean isAnchor) {
            ...
            if (keylineEnd > this.carouselSize) {
                float cutoff2 = Math.abs(keylineEnd - Math.max(keylineEnd - maskedItemSize, this.carouselSize));
                cutoff = cutoff2;
            } else if (keylineStart < 0.0f) {
                float cutoff3 = Math.abs(keylineStart - Math.min(keylineStart + maskedItemSize, 0.0f));
                cutoff = cutoff3;
            } else {
                cutoff = 0.0f;
            }
