...
public final class DispatchedContinuation<T> extends DispatchedTask<T> implements CoroutineStackFrame, Continuation<T> {
        ...
        do {
            ...
            if (state != DispatchedContinuationKt.REUSABLE_CLAIMED) {
                if (state instanceof Throwable) {
                    if (!AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(_reusableCancellableContinuation$volatile$FU, this, state, null)) {
    ...
    public void resumeWith(Object result) {
        ...
        if (this.dispatcher.isDispatchNeeded(getContext())) {
            this._state = state;
            this.resumeMode = 0;
            this.dispatcher.mo10498dispatch(getContext(), this);
            return;
        }
        if (DebugKt.getASSERTIONS_ENABLED()) {
        }
    ...
    */
    ...
    public final boolean resumeCancelled$kotlinx_coroutines_core(Object state) {
        ...
        if (job != null && !job.isActive()) {
            ...
            resumeWith(Result.m8929constructorimpl(ResultKt.createFailure(cause)));
            return true;
        }
        return false;
    }
