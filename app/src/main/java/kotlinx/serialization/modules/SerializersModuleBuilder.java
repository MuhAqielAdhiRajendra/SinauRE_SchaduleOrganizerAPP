...
public final class SerializersModuleBuilder implements SerializersModuleCollector {
    ...
    public final <Base, Sub extends Base> void registerPolymorphicSerializer(KClass<Base> baseClass, KClass<Sub> concreteClass, KSerializer<Sub> concreteSerializer, boolean allowOverwrite) {
        ...
        Map<KClass<?>, Map<KClass<?>, KSerializer<?>>> map3 = this.polyBase2Serializers;
        Map<KClass<?>, KSerializer<?>> map4 = map3.get(baseClass);
        if (map4 == null) {
            map = new HashMap<>();
            map3.put(baseClass, map);
        } else {
            map = map4;
        }
        Map<KClass<?>, KSerializer<?>> map5 = map;
        KSerializer<?> kSerializer = map5.get(concreteClass);
        Map<KClass<?>, Map<String, KSerializer<?>>> map6 = this.polyBase2NamedSerializers;
        Map<String, KSerializer<?>> map7 = map6.get(baseClass);
        if (map7 == null) {
            map2 = new HashMap<>();
            map6.put(baseClass, map2);
        } else {
            map2 = map7;
        }
        ...
        if (allowOverwrite) {
            if (kSerializer != null) {
                map8.remove(kSerializer.getDescriptor().getSerialName());
            }
            map5.put(concreteClass, concreteSerializer);
            map8.put(name, concreteSerializer);
            return;
        }
        if (kSerializer != null) {
        ...
        KSerializer<?> kSerializer2 = map8.get(name);
        if (kSerializer2 == null) {
            map5.put(concreteClass, concreteSerializer);
            map8.put(name, concreteSerializer);
            return;
        }
        Map<KClass<?>, KSerializer<?>> map9 = this.polyBase2Serializers.get(baseClass);
