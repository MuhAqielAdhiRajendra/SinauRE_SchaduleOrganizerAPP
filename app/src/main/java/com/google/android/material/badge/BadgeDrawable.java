...
public class BadgeDrawable extends Drawable implements TextDrawableHelper.TextDrawableDelegate {
    ...
    private int getTotalHorizontalOffsetForState() {
        ...
        BadgeState badgeState = this.state;
        if (zHasBadgeContent) {
            hOffset = badgeState.getHorizontalOffsetWithText();
        } else {
            hOffset = badgeState.getHorizontalOffsetWithoutText();
        }
        if (this.state.offsetAlignmentMode == 1) {
            boolean zHasBadgeContent2 = hasBadgeContent();
            BadgeState badgeState2 = this.state;
            hOffset += zHasBadgeContent2 ? badgeState2.horizontalInsetWithText : badgeState2.horizontalInset;
        }
    ...
    private void calculateCenterAndBounds(Rect anchorRect, View anchorView) {
        ...
        boolean zHasBadgeContent = hasBadgeContent();
        BadgeState badgeState = this.state;
        this.cornerRadius = zHasBadgeContent ? badgeState.badgeWithTextRadius : badgeState.badgeRadius;
        if (this.cornerRadius != -1.0f) {
        ...
        BadgeState badgeState4 = this.state;
        switch (badgeGravity) {
            case 8388659:
            case 8388691:
                if (badgeState4.badgeFixedEdge == 0) {
                    if (anchorView.getLayoutDirection() == 0) {
                        f = (anchorRect.left + this.halfBadgeWidth) - ((this.halfBadgeHeight * 2.0f) - totalHorizontalOffset);
                    } else {
                        f = (anchorRect.right - this.halfBadgeWidth) + ((this.halfBadgeHeight * 2.0f) - totalHorizontalOffset);
                    }
                } else if (anchorView.getLayoutDirection() == 0) {
                    f = totalHorizontalOffset + (anchorRect.left - this.halfBadgeWidth);
                } else {
            ...
            default:
                if (badgeState4.badgeFixedEdge == 0) {
                    ...
                    f2 = (anchorRect.right - this.halfBadgeWidth) + ((this.halfBadgeHeight * 2.0f) - totalHorizontalOffset);
                } else {
                    f2 = (anchorRect.left + this.halfBadgeWidth) - ((this.halfBadgeHeight * 2.0f) - totalHorizontalOffset);
                }
