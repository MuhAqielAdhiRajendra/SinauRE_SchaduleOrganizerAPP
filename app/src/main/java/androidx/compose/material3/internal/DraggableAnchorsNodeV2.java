...
final class DraggableAnchorsNodeV2<T> extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo67measure3p2s80s(final MeasureScope $this$measure_u2d3p2s80s, Measurable measurable, long constraints) {
        ...
        if (!$this$measure_u2d3p2s80s.isLookingAhead() || !this.didLookahead) {
            ...
            Pair<? extends androidx.compose.foundation.gestures.DraggableAnchors<T>, ? extends T> pairInvoke = this.anchors.invoke(IntSize.m8313boximpl(size), Constraints.m8090boximpl(constraints));
        ...
        return MeasureScope.layout$default($this$measure_u2d3p2s80s, placeable.getWidth(), placeable.getHeight(), null, new Function1() { // from class: androidx.compose.material3.internal.DraggableAnchorsNodeV2$$ExternalSyntheticLambda0
            @Override // kotlin.jvm.functions.Function1
            public final Object invoke(Object obj) {
                return DraggableAnchorsNodeV2.measure_3p2s80s$lambda$1($this$measure_u2d3p2s80s, this, placeable, (Placeable.PlacementScope) obj);
            }
        }, 4, null);
    }

    static final Unit measure_3p2s80s$lambda$1(MeasureScope measureScope, DraggableAnchorsNodeV2 draggableAnchorsNodeV2, final Placeable placeable, Placeable.PlacementScope placementScope) {
        float fRequireOffset;
        if (measureScope.isLookingAhead()) {
            fRequireOffset = draggableAnchorsNodeV2.state.getAnchors().positionOf(draggableAnchorsNodeV2.state.getTargetValue());
        } else {
            fRequireOffset = draggableAnchorsNodeV2.state.requireOffset();
        }
        final float f = draggableAnchorsNodeV2.orientation == Orientation.Horizontal ? fRequireOffset * (draggableAnchorsNodeV2.isReverseDirection() ? -1.0f : 1.0f) : 0.0f;
        final float f2 = draggableAnchorsNodeV2.orientation == Orientation.Vertical ? fRequireOffset : 0.0f;
        placementScope.withMotionFrameOfReferencePlacement(new Function1() { // from class: androidx.compose.material3.internal.DraggableAnchorsNodeV2$$ExternalSyntheticLambda1
            @Override // kotlin.jvm.functions.Function1
            public final Object invoke(Object obj) {
                return DraggableAnchorsNodeV2.measure_3p2s80s$lambda$1$lambda$0(placeable, f, f2, (Placeable.PlacementScope) obj);
            }
        });
