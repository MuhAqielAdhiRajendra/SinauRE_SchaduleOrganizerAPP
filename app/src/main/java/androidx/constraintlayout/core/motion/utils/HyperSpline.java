...
public class HyperSpline {
    ...
    public void setup(double[][] points) {
        ...
        this.mCtl = (double[][]) Array.newInstance((Class<?>) Double.TYPE, this.mDimensionality, this.mPoints);
        ...
        for (int p2 = 0; p2 < this.mCurveLength.length; p2++) {
            ...
            double dApproxLength = approxLength(temp);
            dArr[p2] = dApproxLength;
            this.mTotalLength = d5 + dApproxLength;
        }
    }
    ...
    public double approxLength(Cubic[] curve) {
        ...
        for (double i = 0.0d; i < 1.0d; i += 0.1d) {
            ...
            for (int j = 0; j < n; j++) {
                double tmp = old[j];
                double dEval = curve[j].eval(i);
                old[j] = dEval;
                double tmp2 = tmp - dEval;
                s += tmp2 * tmp2;
            }
            if (i > 0.0d) {
                sum += Math.sqrt(s);
        ...
        for (int j2 = 0; j2 < n; j2++) {
            double tmp3 = old[j2];
            double dEval2 = curve[j2].eval(1.0d);
            old[j2] = dEval2;
            double tmp4 = tmp3 - dEval2;
            s2 += tmp4 * tmp4;
        }
