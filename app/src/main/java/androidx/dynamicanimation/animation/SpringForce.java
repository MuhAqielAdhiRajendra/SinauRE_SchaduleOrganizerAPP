...
public final class SpringForce implements Force {
    ...
    private void init() {
        ...
        if (d > 1.0d) {
            this.mGammaPlus = ((-d2) * this.mNaturalFreq) + (this.mNaturalFreq * Math.sqrt((this.mDampingRatio * this.mDampingRatio) - 1.0d));
        ...
        } else if (d2 >= 0.0d && this.mDampingRatio < 1.0d) {
    ...
    DynamicAnimation.MassState updateValues(double lastDisplacement, double lastVelocity, long timeElapsed) {
        ...
        if (this.mDampingRatio > 1.0d) {
            ...
            displacement = Math.pow(2.718281828459045d, (-this.mDampingRatio) * this.mNaturalFreq * deltaT) * ((Math.cos(this.mDampedFreq * deltaT) * lastDisplacement2) + (Math.sin(this.mDampedFreq * deltaT) * sinCoeff));
            ...
            currentVelocity = d + (dPow * ((d2 * Math.sin(cosCoeff * deltaT)) + (Math.cos(this.mDampedFreq * deltaT) * this.mDampedFreq * sinCoeff)));
        } else {
            double coeffB2 = lastVelocity + (this.mNaturalFreq * lastDisplacement2);
            displacement = ((coeffB2 * deltaT) + lastDisplacement2) * Math.pow(2.718281828459045d, (-this.mNaturalFreq) * deltaT);
