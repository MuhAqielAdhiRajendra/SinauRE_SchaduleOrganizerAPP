...
public abstract class WindowAreaController {
    ...
    static final WindowAreaController windowAreaController_delegate$lambda$1() {
        ...
        try {
            ClassLoader it = INSTANCE.getClass().getClassLoader();
            if (it != null) {
                windowAreaComponentExtensions = new SafeWindowAreaComponentProvider(it).getWindowAreaComponent();
            }
        } catch (Throwable th) {
        ...
        boolean deviceSupported = Build.VERSION.SDK_INT > 29 && windowAreaComponentExtensions != null && ExtensionsUtil.INSTANCE.getSafeVendorApiLevel() >= 3;
        if (deviceSupported) {
            Intrinsics.checkNotNull(windowAreaComponentExtensions);
            return new WindowAreaControllerImpl(windowAreaComponentExtensions);
        }
    ...
    @JvmStatic
    public static final void overrideDecorator(WindowAreaControllerDecorator overridingDecorator) {
