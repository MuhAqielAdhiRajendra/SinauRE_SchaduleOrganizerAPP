...
public class AsyncListUtil<T> {
    ...
    private final ThreadUtil.MainThreadCallback<T> mMainThreadCallback = new ThreadUtil.MainThreadCallback<T>() { // from class: androidx.recyclerview.widget.AsyncListUtil.1
        ...
        public void addTile(int generation, TileList.Tile<T> tile) {
            boolean zIsRequestedGeneration = isRequestedGeneration(generation);
            AsyncListUtil asyncListUtil = AsyncListUtil.this;
            if (!zIsRequestedGeneration) {
                asyncListUtil.mBackgroundProxy.recycleTile(tile);
                return;
            }
            TileList.Tile<T> duplicate = asyncListUtil.mTileList.addOrReplace(tile);
            if (duplicate != null) {
        ...
        private void recycleAllTiles() {
            ...
            while (true) {
                ...
                AsyncListUtil asyncListUtil = AsyncListUtil.this;
                if (i < size) {
                    asyncListUtil.mBackgroundProxy.recycleTile(AsyncListUtil.this.mTileList.getAtIndex(i));
                    i++;
                } else {
