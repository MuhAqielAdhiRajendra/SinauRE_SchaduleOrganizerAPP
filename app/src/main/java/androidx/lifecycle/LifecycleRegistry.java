...
public class LifecycleRegistry extends Lifecycle {
    ...
    private final boolean isSynced() {
        ...
        Lifecycle.State eldestObserverState = entryEldest.getValue().getState();
        Map.Entry<LifecycleObserver, ObserverWithState> entryNewest = this.observerMap.newest();
        Intrinsics.checkNotNull(entryNewest);
        Lifecycle.State newestObserverState = entryNewest.getValue().getState();
    ...
    private final Lifecycle.State calculateTargetState(LifecycleObserver observer) {
        ...
        Lifecycle.State siblingState = (entryCeil == null || (value = entryCeil.getValue()) == null) ? null : value.getState();
        Lifecycle.State parentState = this.parentStates.isEmpty() ? null : this.parentStates.get(this.parentStates.size() - 1);
        return INSTANCE.min$lifecycle_runtime(INSTANCE.min$lifecycle_runtime(this.state, siblingState), parentState);
    }
    ...
    private final void forwardPass(LifecycleOwner lifecycleOwner) {
        ...
        Intrinsics.checkNotNullExpressionValue(itIteratorWithAdditions, "iteratorWithAdditions(...)");
        Iterator ascendingIterator = itIteratorWithAdditions;
    ...
    public static final class ObserverWithState {
        ...
        public final void dispatchEvent(LifecycleOwner owner, Lifecycle.Event event) {
            ...
            this.state = LifecycleRegistry.INSTANCE.min$lifecycle_runtime(this.state, newState);
    ...
    @JvmStatic
    public static final LifecycleRegistry createUnsafe(LifecycleOwner owner) {
        return INSTANCE.createUnsafe(owner);
    }
}
