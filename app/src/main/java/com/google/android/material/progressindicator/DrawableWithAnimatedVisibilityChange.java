...
abstract class DrawableWithAnimatedVisibilityChange extends Drawable implements Animatable2Compat {
    ...
    void setMockPhaseFraction(float fraction) {
    ...
    float getPhaseFraction() {
        ...
        BaseProgressIndicatorSpec baseProgressIndicatorSpec = this.baseSpec;
        if (zIsDeterminateDrawable) {
            wavelength = baseProgressIndicatorSpec.wavelengthDeterminate;
        } else {
            wavelength = baseProgressIndicatorSpec.wavelengthIndeterminate;
        }
        int cycleInMs = (int) (((wavelength * 1000.0f) / this.baseSpec.waveSpeed) * durationScale);
        float phaseFraction = (SystemClock.uptimeMillis() % ((long) cycleInMs)) / cycleInMs;
