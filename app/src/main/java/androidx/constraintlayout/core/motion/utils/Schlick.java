...
public class Schlick extends Easing {
    ...
    private double func(double x) {
        ...
        double d2 = this.mT;
        if (x < d) {
            return (d2 * x) / ((this.mS * (this.mT - x)) + x);
        }
        return ((1.0d - d2) * (x - 1.0d)) / ((1.0d - x) - (this.mS * (this.mT - x)));
    }
    ...
    private double dfunc(double x) {
        ...
        double d2 = this.mS;
        if (x < d) {
            return ((d2 * this.mT) * this.mT) / (((this.mS * (this.mT - x)) + x) * ((this.mS * (this.mT - x)) + x));
        }
        return ((d2 * (this.mT - 1.0d)) * (this.mT - 1.0d)) / (((((-this.mS) * (this.mT - x)) - x) + 1.0d) * ((((-this.mS) * (this.mT - x)) - x) + 1.0d));
    }
