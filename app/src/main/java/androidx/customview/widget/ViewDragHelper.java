...
public class ViewDragHelper {
    ...
    private ViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        ...
        this.mDefaultEdgeSize = (int) ((20.0f * density) + 0.5f);
        ...
        Interpolator delegatingInterpolator = new Interpolator() { // from class: androidx.customview.widget.ViewDragHelper.3
            @Override // android.animation.TimeInterpolator
            public float getInterpolation(float input) {
                return ViewDragHelper.this.mInterpolator.getInterpolation(input);
            }
        };
        this.mScroller = new OverScroller(context, delegatingInterpolator);
    }
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action2) {
                ...
                if (this.mInitialMotionX == null || this.mInitialMotionY == null) {
                    ...
                    while (i < pointerCount2) {
                        ...
                        if (isValidPointerForActionMove(pointerId2)) {
                            ...
                            boolean pastSlop = checkTouchSlop(toCapture3, dx, dy);
                            if (!pastSlop) {
                                action = action2;
                            ...
                            if (this.mDragState == 1 || (pastSlop && tryCaptureViewForDrag(toCapture3, pointerId2))) {
                                saveLastMotion(ev);
                                z = false;
                                break;
                            }
            ...
            case 5:
                ...
                if (this.mDragState != 0) {
                    if (this.mDragState == 2 && (toCapture = findTopChildUnder((int) x3, (int) y3)) == this.mCapturedView) {
    ...
    public void processTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case 2:
                if (this.mDragState == 1) {
                    if (isValidPointerForActionMove(this.mActivePointerId) && (index = ev.findPointerIndex(this.mActivePointerId)) != -1) {
    ...
    public boolean checkTouchSlop(int directions, int pointerId) {
        ...
        if (this.mInitialMotionX == null || this.mInitialMotionY == null || this.mLastMotionX == null || this.mLastMotionY == null) {
            Log.w(TAG, "Inconsistent pointer event stream: pointer is down, but there is no initial motion recorded. Is something intercepting or modifying events?");
            return false;
        }
    ...
    public boolean isEdgeTouched(int edges) {
        if (this.mInitialEdgesTouched == null) {
            return false;
        }
    ...
    public boolean isEdgeTouched(int edges, int pointerId) {
        return (!isPointerDown(pointerId) || this.mInitialEdgesTouched == null || (this.mInitialEdgesTouched[pointerId] & edges) == 0) ? false : true;
    }
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        ...
        if (dx != 0) {
            int clampedX2 = this.mCallback.clampViewPositionHorizontal(this.mCapturedView, left, dx);
            ViewCompat.offsetLeftAndRight(this.mCapturedView, clampedX2 - oldLeft);
            clampedX = clampedX2;
        } else {
            clampedX = left;
        }
        if (dy != 0) {
            int clampedY2 = this.mCallback.clampViewPositionVertical(this.mCapturedView, top, dy);
            ViewCompat.offsetTopAndBottom(this.mCapturedView, clampedY2 - oldTop);
            clampedY = clampedY2;
        } else {
            clampedY = top;
        }
