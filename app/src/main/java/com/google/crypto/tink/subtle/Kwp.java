...
public class Kwp implements KeyWrap {
    ...
    private byte[] computeW(final byte[] iv, final byte[] key) throws GeneralSecurityException {
        if (key.length > 8 && key.length <= 2147483631 && iv.length == 8) {
            ...
            while (i3 < 6) {
                int j = 0;
                while (j < blocks) {
                    System.arraycopy(data, (j + 1) * 8, block, 8, 8);
                    int length = aes.doFinal(block, 0, i2, block);
                    if (length != i2) {
                        throw new AssertionError();
                    }
                    int roundConst = (i3 * blocks) + j + i;
                    for (int b = 0; b < 4; b++) {
                        int i4 = 7 - b;
                        block[i4] = (byte) (block[i4] ^ ((byte) (roundConst & 255)));
                        roundConst >>>= 8;
                    }
                    System.arraycopy(block, 8, data, (j + 1) * 8, 8);
                    j++;
                    i = 1;
                    i2 = 16;
                }
    ...
    private byte[] invertW(final byte[] wrapped) throws GeneralSecurityException {
        ...
        for (int i = 5; i >= 0; i--) {
            for (int j = blocks - 1; j >= 0; j--) {
                ...
                for (int b = 0; b < 4; b++) {
                    int i2 = 7 - b;
                    block[i2] = (byte) (block[i2] ^ ((byte) (roundConst & 255)));
                    roundConst >>>= 8;
                }
    ...
    public byte[] wrap(final byte[] data) throws GeneralSecurityException {
        ...
        for (int i = 0; i < 4; i++) {
            iv[i + 4] = (byte) ((data.length >> ((3 - i) * 8)) & 255);
        }
    ...
    public byte[] unwrap(final byte[] data) throws GeneralSecurityException {
        ...
        for (int i2 = 4; i2 < 8; i2++) {
            encodedSize = (encodedSize << 8) + (unwrapped[i2] & UByte.MAX_VALUE);
        }
        int i3 = wrappingSize(encodedSize);
        if (i3 != unwrapped.length) {
            ok = false;
        } else {
            for (int j = encodedSize + 8; j < unwrapped.length; j++) {
                if (unwrapped[j] != 0) {
                    ok = false;
                }
            }
        ...
        if (ok) {
            return Arrays.copyOfRange(unwrapped, 8, encodedSize + 8);
        }
