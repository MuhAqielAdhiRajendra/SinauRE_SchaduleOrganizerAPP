...
public final class FontScaleConverterFactory {
    ...
    static {
        ...
        if ((minScaleBeforeCurvesApplied > MinScaleForNonLinear ? 1 : 0) == 0) {
    ...
    private final void put(float scaleKey, FontScaleConverter fontScaleConverter) {
        synchronized (LookupTablesWriteLock) {
            SparseArrayCompat<FontScaleConverter> sparseArrayCompatM32clone = sLookupTables.m32clone();
            INSTANCE.putInto(sparseArrayCompatM32clone, scaleKey, fontScaleConverter);
            sLookupTables = sparseArrayCompatM32clone;
            Unit unit = Unit.INSTANCE;
        }
    }
