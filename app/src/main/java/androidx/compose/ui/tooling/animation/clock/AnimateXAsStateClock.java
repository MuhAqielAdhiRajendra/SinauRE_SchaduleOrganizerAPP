...
public final class AnimateXAsStateClock<T, V extends AnimationVector> implements ComposeAnimationClock<AnimateXAsStateComposeAnimation<T, V>, TargetState<T>> {
    ...
    private TargetState<T> state = new TargetState<>(getAnimation().m8070getAnimationObject().getValue(), getAnimation().m8070getAnimationObject().getValue());
    ...
    public void setStateParameters(Object par1, Object par2) {
        TargetState<T> parametersToValue = Utils_androidKt.parseParametersToValue(this.currentValue, par1, par2);
    ...
    private final TargetBasedAnimation<T, V> getCurrentAnimation() {
        return AnimationKt.TargetBasedAnimation(getAnimation().getAnimationSpec(), getAnimation().m8070getAnimationObject().getTypeConverter(), getState().getInitial(), getState().getTarget(), getAnimation().m8070getAnimationObject().getVelocity());
    }
}
