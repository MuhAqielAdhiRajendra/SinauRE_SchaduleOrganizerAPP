...
final class BackStackRecord extends FragmentTransaction implements FragmentManager.BackStackEntry, FragmentManager.OpGenerator {
    ...
    /* JADX WARN: Illegal instructions before constructor call */
    BackStackRecord(BackStackRecord bse) {
        ClassLoader classLoader;
        FragmentFactory fragmentFactory = bse.mManager.getFragmentFactory();
        if (bse.mManager.getHost() != null) {
            classLoader = bse.mManager.getHost().getContext().getClassLoader();
        } else {
            classLoader = null;
        }
        super(fragmentFactory, classLoader, bse);
        this.mIndex = -1;
        this.mBeingSaved = false;
