...
public final class BoundsAnimation {
    ...
    public BoundsAnimation(SharedTransitionScope transitionScope, Transition<Boolean> transition, Transition<Boolean>.DeferredAnimation<Rect, AnimationVector4D> deferredAnimation, BoundsTransform boundsTransform, Function0<Offset> function0) {
    ...
    public final Rect getValue() {
        ...
        Rect it;
        if (!this.transitionScope.isTransitionActive() || (animationState = getAnimationState()) == null || (it = animationState.getValue()) == null) {
            return null;
        }
        long offset = this.momentumOffset.invoke().m5078unboximpl();
        if (!Offset.m5065equalsimpl0(offset, Offset.INSTANCE.m5084getZeroF1C5BW0())) {
            return it.m5105translatek4lQ0M(offset);
        }
        return it;
    }
    ...
    public final void animate(final Rect currentBounds, final Rect targetBounds, BoundsTransform forcedBoundsTransform) {
        if (this.transitionScope.isTransitionActive()) {
            if (getAnimationState() == null) {
                this.animationSpec = (forcedBoundsTransform == null ? getBoundsTransform() : forcedBoundsTransform).createAnimationSpec(currentBounds, targetBounds);
            }
