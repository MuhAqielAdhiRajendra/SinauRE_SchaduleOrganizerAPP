...
public final class ColorSpaceKt {
    ...
    private static final Connector m5756createConnectorYBCOT_4(ColorSpace source, ColorSpace destination, int intent) {
        if (source == destination) {
            return Connector.INSTANCE.identity$ui_graphics(source);
        }
        DefaultConstructorMarker defaultConstructorMarker = null;
        if (ColorModel.m5743equalsimpl0(source.getModel(), ColorModel.INSTANCE.m5750getRgbxdoWZVw()) && ColorModel.m5743equalsimpl0(destination.getModel(), ColorModel.INSTANCE.m5750getRgbxdoWZVw())) {
    ...
    public static /* synthetic */ Connector m5755connectYBCOT_4$default(ColorSpace colorSpace, ColorSpace colorSpace2, int i, int i2, Object obj) {
        if ((i2 & 1) != 0) {
            colorSpace2 = ColorSpaces.INSTANCE.getSrgb();
        }
        if ((i2 & 2) != 0) {
            i = RenderIntent.INSTANCE.m5771getPerceptualuksYyKA();
        }
        return m5754connectYBCOT_4(colorSpace, colorSpace2, i);
    }

    /* JADX INFO: renamed from: connect-YBCOT_4, reason: not valid java name */
    public static final Connector m5754connectYBCOT_4(ColorSpace $this$connect_u2dYBCOT_4, ColorSpace destination, int intent) {
        int srcId = $this$connect_u2dYBCOT_4.getId();
        int dstId = destination.getId();
        if ((srcId | dstId) < 0) {
            return m5756createConnectorYBCOT_4($this$connect_u2dYBCOT_4, destination, intent);
        }
        MutableIntObjectMap<Connector> connectors = ConnectorKt.getConnectors();
        int renderIntent$iv = (dstId << 6) | srcId | (intent << 12);
        Connector connectorM5756createConnectorYBCOT_4 = connectors.get(renderIntent$iv);
        if (connectorM5756createConnectorYBCOT_4 == null) {
            connectorM5756createConnectorYBCOT_4 = m5756createConnectorYBCOT_4($this$connect_u2dYBCOT_4, destination, intent);
            connectors.set(renderIntent$iv, connectorM5756createConnectorYBCOT_4);
        }
        return connectorM5756createConnectorYBCOT_4;
    }
    ...
    public static final ColorSpace adapt(ColorSpace $this$adapt, WhitePoint whitePoint, Adaptation adaptation) {
        if (ColorModel.m5743equalsimpl0($this$adapt.getModel(), ColorModel.INSTANCE.m5750getRgbxdoWZVw())) {
            ...
            float[] adaptationTransform = chromaticAdaptation(adaptation.getTransform(), rgb.getWhitePoint().toXyz$ui_graphics(), xyz);
