...
class StreamingAeadSeekableDecryptingChannel implements SeekableByteChannel {
    ...
    private int getSegmentNr(long plaintextPosition) {
        return (int) ((((long) this.ciphertextOffset) + plaintextPosition) / ((long) this.plaintextSegmentSize));
    }
    ...
    public synchronized int read(ByteBuffer dst) throws IOException {
        ...
        while (dst.remaining() > 0 && this.plaintextPosition < this.plaintextSize) {
            ...
            if (segmentNr == 0) {
                ...
                segmentOffset = (int) ((j + ((long) this.ciphertextOffset)) % ((long) this.plaintextSegmentSize));
            }
