...
public class DependencyGraph {
    ...
    public boolean directMeasureWithOrientation(boolean optimizeWrap, int orientation) {
        ...
        if (orientation != 0) {
            if (constraintWidgetContainer.mListDimensionBehaviors[1] == ConstraintWidget.DimensionBehaviour.FIXED || this.mWidgetcontainer.mListDimensionBehaviors[1] == ConstraintWidget.DimensionBehaviour.MATCH_PARENT) {
                int y2 = this.mWidgetcontainer.getHeight() + y1;
                this.mWidgetcontainer.mVerticalRun.end.resolve(y2);
                this.mWidgetcontainer.mVerticalRun.mDimension.resolve(y2 - y1);
                checkRoot = true;
            }
        } else if (constraintWidgetContainer.mListDimensionBehaviors[0] == ConstraintWidget.DimensionBehaviour.FIXED || this.mWidgetcontainer.mListDimensionBehaviors[0] == ConstraintWidget.DimensionBehaviour.MATCH_PARENT) {
            int x2 = this.mWidgetcontainer.getWidth() + x1;
            this.mWidgetcontainer.mHorizontalRun.end.resolve(x2);
            this.mWidgetcontainer.mHorizontalRun.mDimension.resolve(x2 - x1);
            checkRoot = true;
    ...
    */
    ...
    public void measureWidgets() {
        ...
        for (ConstraintWidget widget : dependencyGraph.mWidgetcontainer.mChildren) {
            if (!widget.measured) {
                boolean z = false;
    ...
    public void buildGraph(ArrayList<WidgetRun> runs) {
        ...
        Iterator<WidgetRun> it = runs.iterator();
        while (it.hasNext()) {
            it.next().clear();
        }
