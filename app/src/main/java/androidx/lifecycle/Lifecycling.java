...
public final class Lifecycling {
    ...
    public static final LifecycleEventObserver lifecycleEventObserver(Object object) {
        ...
        int type = INSTANCE.getObserverConstructorType(cls);
        if (type == 2) {
            ...
            if (list2.size() == 1) {
                GeneratedAdapter generatedAdapter = INSTANCE.createGeneratedAdapter(list2.get(0), object);
                return new SingleGeneratedAdapterObserver(generatedAdapter);
            }
            int size = list2.size();
            GeneratedAdapter[] adapters = new GeneratedAdapter[size];
            for (int i = 0; i < size; i++) {
                adapters[i] = INSTANCE.createGeneratedAdapter(list2.get(i), object);
            }
    ...
    private final Constructor<? extends GeneratedAdapter> generatedConstructor(Class<?> klass) {
        ...
        try {
            ...
            if (fullPackage.length() == 0) {
                ...
                strSubstring = name.substring(fullPackage.length() + 1);
                Intrinsics.checkNotNullExpressionValue(strSubstring, "substring(...)");
            }
            Intrinsics.checkNotNull(strSubstring);
            String adapterName = getAdapterName(strSubstring);
            Class<?> cls = Class.forName(fullPackage.length() == 0 ? adapterName : fullPackage + '.' + adapterName);
            Intrinsics.checkNotNull(cls, "null cannot be cast to non-null type java.lang.Class<out androidx.lifecycle.GeneratedAdapter>");
            Constructor constructor = cls.getDeclaredConstructor(klass);
