...
final class LookaheadCapablePlacementScope extends Placeable.PlacementScope {
    ...
    public float current(Ruler $this$current, float defaultValue) {
        if ($this$current.getCalculate$ui() != null) {
            return $this$current.getCalculate$ui().invoke(this, Float.valueOf(defaultValue)).floatValue();
        }
    ...
    public float get_density() {
        return this.within.get_density();
    }
    ...
    public float get_fontScale() {
        return this.within.get_fontScale();
    }
}
