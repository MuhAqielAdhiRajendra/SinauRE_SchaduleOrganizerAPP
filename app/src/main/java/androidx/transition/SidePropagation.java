...
public class SidePropagation extends VisibilityPropagation {
    ...
    public long getStartDelay(ViewGroup sceneRoot, Transition transition, TransitionValues startValues, TransitionValues endValues) {
        ...
        int right = sceneRoot.getWidth() + left;
        int bottom = sceneRoot.getHeight() + top;
        if (epicenter != null) {
            epicenterX2 = epicenter.centerX();
            epicenterX = epicenter.centerY();
        } else {
            int epicenterY = left + right;
            int epicenterX3 = epicenterY / 2;
            epicenterX = (top + bottom) / 2;
            epicenterX2 = epicenterX3;
        }
        int epicenterY2 = epicenterX;
        float distance = distance(sceneRoot, viewCenterX, viewCenterY, epicenterX2, epicenterY2, left, top, right, bottom);
        ...
        return Math.round(((((long) directionMultiplier) * duration) / this.mPropagationSpeed) * distanceFraction);
    }
