...
public final class ViewingConditions {
    ...
    public static ViewingConditions make(double[] whitePoint, double adaptingLuminance, double backgroundLstar, double surround, boolean discountingIlluminant) {
        ...
        double c = f >= 0.9d ? MathUtils.lerp(0.59d, 0.69d, (f - 0.9d) * 10.0d) : MathUtils.lerp(0.525d, 0.59d, (f - 0.8d) * 10.0d);
        if (discountingIlluminant) {
            d = 0.1d;
            d2 = 1.0d;
        } else {
            d = 0.1d;
            d2 = (1.0d - (Math.exp(((-adaptingLuminance) - 42.0d) / 92.0d) * 0.2777777777777778d)) * f;
        }
        double d3 = MathUtils.clampDouble(0.0d, 1.0d, d2);
        double[] rgbD = {(((100.0d / rW) * d3) + 1.0d) - d3, (((100.0d / gW) * d3) + 1.0d) - d3, (((100.0d / bW) * d3) + 1.0d) - d3};
        double k = 1.0d / ((adaptingLuminance * 5.0d) + 1.0d);
        double k4 = k * k * k * k;
        double k4F = 1.0d - k4;
        double fl = (k4 * adaptingLuminance) + (d * k4F * k4F * Math.cbrt(5.0d * adaptingLuminance));
